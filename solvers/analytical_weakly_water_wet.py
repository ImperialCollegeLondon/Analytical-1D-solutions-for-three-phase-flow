...
def _find_M_on_boundary(L, R, cp, f2, ns=400):
    """
    Strategy A for L on the So=SORG boundary.  DISABLED for now:
    returns None so `find_intermediate_state` falls through to the
    Newton / curve-intersection / transitional-shock strategies.

    Background
    ----------
    When L sits on the oil edge, our Baker+phi(So) model has a slow
    eigenvector at L that is exactly tangent to that edge, so the slow
    integral curve from L is stuck on the boundary.  A proper boundary
    Welge tangent construction to match the fast wave structure on the
    interior side would need to account for non-genuine nonlinearity
    (inflection locus) and transitional / undercompressive shocks,
    which is beyond the classical Juanes-Patzek nine-type catalogue.
    Until that machinery is implemented we defer to Strategy 1b/1c/1g.
    """
    # Disabled: see docstring above.
    return None

    # ------- unreachable legacy body kept for future Welge rework -------
...
def _find_fast_composite(L, M, R, cp, f2):
    ...
    for direction, sub in branches:
        ...
        if sign_change is None:
            ...
            cand = (cand_res, H_cand, direction, sub, sigs[kbest], sub[kbest, 3])
        ...
        cand = (cand_res, H_cand, direction, sub, sigs[i_best], sub[i_best, 3])
    ...
    abs_res, H, direction, sub, sig_LH, lam2_H = best

    # Quality gate: residual must really be small
    if abs_res > 0.05:
        print(f"      [1e] best |residual|={abs_res:.4f} > 0.05; "
              f"fast composite does not exist for this problem")
        return None, None, None

    # Reject degenerate H = L (trivial Welge zero at curve endpoint).
    # When the fast integral curve from M happens to pass through L, the
    # residual sigma(L,P) - lambda2(P) is exactly 0 at P=L by construction
    # (the "shock" L->L has zero amplitude and sigma degenerates to lambda2).
    # Accepting this root gives a spurious "pure fast rarefaction L->M"
    # whose integral curve is actually a non-monotone fold crossing
    # multiple inflection loci, not an admissible rarefaction.
    d_HL = float(np.sqrt((H[0] - L[0])**2 + (H[2] - L[2])**2))
    if d_HL < 0.02:
        print(f"      [1e] rejected degenerate H ~ L (|H-L|={d_HL:.2e}); "
              f"no genuine Welge tangent exists (curve passes through L)")
        return None, None, None

    # Also reject if the shock L->H has near-zero amplitude in lambda_2
    # (sig_LH must be strictly greater than lambda_2(L) for a real shock).
    try:
        _, lam2_L, _, _, _ = compute_eigenvalues(L[0], L[2], cp)
        if sig_LH <= lam2_L + 1e-4:
            print(f"      [1e] rejected degenerate shock "
                  f"sigma(L,H)={sig_LH:.4f} <= lambda2(L)={lam2_L:.4f}")
            return None, None, None
    except Exception:
        pass
...
def _find_J1_on_boundary_to_M(L, M, cp, f2, ns=600,
                              score_tol=0.30, rh_tol=0.05):
    """
    Find J1 on the So=SORG (gas-water) edge such that the L->J1 boundary
    BL rarefaction joins continuously with the transitional shock J1->M:

        sigma(J1, M)  =  lambda_edge(J1) = dfw/dSw |_{J1, on edge}

    The shock J1->M leaves the edge and crosses the elliptic / non-
    hyperbolic region into the interior state M.  Returns
    (J1, sigma_J1M, lam_edge_J1, info) or four Nones.
    """
    print("    Strategy boundary-R1: looking for J1 on So=SORG edge ...")
    Sw_L = float(L[0])
    Sw_arr = np.linspace(SWC + 0.002, 1.0 - SORG - 0.002, ns)
    # Boundary BL slow rarefaction can only go forward in Sw (gas displacing
    # water on the gas-water edge), so J1 must be at higher Sw than L.
    cand_mask = Sw_arr > Sw_L + 0.005
    if not cand_mask.any():
        print("      [bndry] no Sw values above L on the edge")
        return None, None, None, None
    Sw_cand = Sw_arr[cand_mask]

    dfw_cand = np.array([_boundary_wave_speed(s, cp, f2) for s in Sw_cand])
    fw_cand = np.array([_boundary_fw(s, cp, f2) for s in Sw_cand])
    fw_L = _boundary_fw(Sw_L, cp, f2)
    dfw_L = _boundary_wave_speed(Sw_L, cp, f2)

    # ---- J1 selection: solve  lam_edge(Sw_J) == lam_2(M)  on the rising
    # side of the BL fan.  This makes the slow rarefaction's trailing
    # characteristic coincide with the fast wave's leading characteristic
    # at M, so the whole self-similar solution has monotone xi:
    #
    #     xi:  lam_edge(L) ----rar---- lam_edge(J1)=lam_2(M)
    #          ----shock---- lam_2(M) ----rar---- lam_2(J2)
    #          ----shock---- sigma(J2,R)
    #
    # The transitional shock J1->M is rendered at xi = lam_2(M) so the
    # slow trailing edge meets the fast leading edge.  Its actual chord
    # speed sigma(J1,M) does NOT need to equal lam_2(M) -- this is a
    # transitional/undercompressive jump whose viscous profile selects a
    # self-similar speed that differs from the inviscid RH chord.
    try:
        _, lam2_M, _, _, _ = compute_eigenvalues(M[0], M[2], cp)
    except Exception:
        return None, None, None, None
    i_peak = int(np.argmax(dfw_cand))
    rising = dfw_cand[:i_peak + 1]
    Sw_rising = Sw_cand[:i_peak + 1]
    if rising.max() < lam2_M:
        # BL fan never reaches lam_2(M).  Place J1 at the peak (end of
        # admissible BL fan) and let the transitional shock cover the rest.
        Sw_J_BL = float(Sw_cand[i_peak])
        match_lam = float(rising[-1])
        print(f"      [bndry] BL peak {match_lam:.4f} < lam_2(M)={lam2_M:.4f}; "
              f"placing J1 at BL peak Sw={Sw_J_BL:.4f}")
    else:
        # Locate Sw_J such that lam_edge(Sw_J) = lam_2(M) on the rising
        # branch.  Linear interp between bracketing samples.
        idx = int(np.searchsorted(rising, lam2_M))
        idx = min(max(idx, 1), len(rising) - 1)
        d0, d1 = rising[idx - 1], rising[idx]
        if d1 > d0 + 1e-12:
            t = (lam2_M - d0) / (d1 - d0)
        else:
            t = 0.0
        Sw_J_BL = float(Sw_rising[idx - 1]
                         + t * (Sw_rising[idx] - Sw_rising[idx - 1]))
        match_lam = float(lam2_M)
        print(f"      [bndry] J1 = lam_edge match to lam_2(M)={lam2_M:.4f} "
              f"at Sw={Sw_J_BL:.4f}  (slow trailing meets fast leading)")
    Sw_BLF = Sw_J_BL

    # Two-pass search:
    # (1) sigma(J,M) and lam_edge(J) along the entire edge; report sign of
    #     residual = sigma - lam_edge across [Sw_L, edge_max].
    # (2) Pick J1 by combined score |residual| + 5*rh, accepting up to
    #     score_tol so that an "approximate" Welge tangent still gives a
    #     usable boundary R1 + transitional shock structure even when the
    #     curves never strictly cross (typical for L on the gas-water edge
    #     where dfw/dSw is small near L compared to chord(L,M) distances).
    sig_arr = np.full(len(Sw_cand), np.nan)
    rh_arr = np.full(len(Sw_cand), np.nan)
    for i, Sw_J in enumerate(Sw_cand):
        Sg_J = 1.0 - Sw_J - SORG
        if Sg_J < 0 or Sg_J > 1:
            continue
        J1 = np.array([Sw_J, SORG, Sg_J])
        sig_J1M, rh_J1M = compute_shock_speed(J1, M, cp, f2)
        if np.isfinite(sig_J1M):
            sig_arr[i] = sig_J1M
            rh_arr[i] = rh_J1M

    res_arr = sig_arr - dfw_cand   # nan-safe element-wise
    valid_mask = np.isfinite(res_arr) & np.isfinite(rh_arr) & (rh_arr < rh_tol)
    if not valid_mask.any():
        # Loosen rh constraint
        valid_mask = np.isfinite(res_arr) & np.isfinite(rh_arr)
    if valid_mask.any():
        sgn = np.sign(res_arr[valid_mask])
        n_pos = int(np.sum(sgn > 0))
        n_neg = int(np.sum(sgn < 0))
        print(f"      [bndry] residual sgn over edge: pos={n_pos}, neg={n_neg}, "
              f"min|res|={np.nanmin(np.abs(res_arr[valid_mask])):.4f}")

    # ---- J1 selection ----
    # Prefer the edge-elliptic boundary point (rar L->J1 along the edge
    # into the elliptic gap, then transitional shock J1->M crossing the
    # gap).  rh(J1,M) is just informational here -- the shock is
    # transitional/undercompressive, not Lax, so RH is what matters and
    # is automatic by construction of compute_shock_speed.
    if Sw_BLF is not None and Sw_BLF > Sw_L + 0.01:
        i_J = int(np.argmin(np.abs(Sw_cand - Sw_BLF)))
        Sw_J = float(Sw_cand[i_J])
        Sg_J = 1.0 - Sw_J - SORG
        J1 = np.array([Sw_J, SORG, Sg_J])
        sig_J1M = float(sig_arr[i_J]) if np.isfinite(sig_arr[i_J]) else float('nan')
        rh_J1M = float(rh_arr[i_J]) if np.isfinite(rh_arr[i_J]) else float('inf')
        welge_res = float(res_arr[i_J]) if np.isfinite(res_arr[i_J]) else float('nan')
        dfw_J = float(dfw_cand[i_J])
        # Score now allows large welge_res (transitional shocks are NOT
        # Lax-tangent); only penalise large rh.
        score = 5.0 * (rh_J1M if np.isfinite(rh_J1M) else 1.0)
        best = (score, Sw_J, J1, sig_J1M, rh_J1M, welge_res, dfw_J)
        print(f"      [bndry] picking J1 at edge-elliptic boundary "
              f"(Sw={Sw_J:.4f}, rh(J1,M)={rh_J1M:.2e})")
    else:
        best = None
        for i, Sw_J in enumerate(Sw_cand):
            if not np.isfinite(sig_arr[i]) or not np.isfinite(rh_arr[i]):
                continue
            if rh_arr[i] > rh_tol:
                continue
            Sg_J = 1.0 - Sw_J - SORG
            J1 = np.array([Sw_J, SORG, Sg_J])
            welge_res = res_arr[i]
            score = abs(welge_res) + 5.0 * rh_arr[i]
            cand = (score, Sw_J, J1, float(sig_arr[i]), float(rh_arr[i]),
                    float(welge_res), float(dfw_cand[i]))
            if best is None or cand[0] < best[0]:
                best = cand

    if best is None:
        print("      [bndry] no J1 candidate satisfies rh < tol")
        return None, None, None, None

    score, Sw_J, J1, sig_J1M, rh_J1M, welge_res, dfw_J = best
    print(f"      [bndry] best J1: Sw={Sw_J:.4f} (Sg={1-Sw_J-SORG:.4f}, "
          f"So={SORG:.2f})")
    print(f"      [bndry]   sigma(J1,M) = {sig_J1M:.4f}  rh = {rh_J1M:.2e}")
    print(f"      [bndry]   lambda_edge(J1) = dfw/dSw = {dfw_J:.4f}")
    print(f"      [bndry]   welge residual = {welge_res:+.4e}  "
          f"score = {score:.4f}")

    # No score veto here -- the boundary R1 + edge-to-interior transitional
    # shock structure is the user-specified ansatz; we trust the projection
    # geometry and let the renderer show whatever rh comes out.
    if not np.isfinite(rh_J1M):
        return None, None, None, None

    # Lax check on the boundary rarefaction L->J1: dfw should be
    # non-decreasing along the edge between Sw_L and Sw_J (so xi = lam_edge
    # is monotone increasing along the rarefaction)
    i_J = int(np.argmin(np.abs(Sw_cand - Sw_J)))
    seg_dfw = dfw_cand[:i_J + 1]
    if len(seg_dfw) >= 2:
        cum_dip = float(np.maximum.accumulate(seg_dfw).max() - seg_dfw.min())
        rng = float(seg_dfw.max() - seg_dfw.min())
        # Allow small numerical noise; the BL fw curve can have an inflection
        # but the slow rarefaction segment should be monotone-increasing in
        # the relevant range.
        non_mono = (np.diff(seg_dfw) < -0.05 * max(rng, 1e-3)).any()
        if non_mono:
            print("      [bndry] BL rarefaction L->J1 is non-monotone in dfw "
                  "(would need a compound on the edge); accepting anyway")

    info = {
        'Sw_J1': Sw_J, 'sigma_J1M': sig_J1M, 'rh_J1M': rh_J1M,
        'lam_edge_J1': dfw_J, 'lam_edge_L': dfw_L,
        'welge_residual': welge_res, 'score': score,
    }
    return J1, sig_J1M, dfw_J, info
...
def _edge_rh_roots_for_Q(Q, L, cp, f2, n=180):
    """All So=0 edge states P that are RH-related to a fixed interior Q."""
    ...
    vals = np.array([_edge_to_state_rh_residual(x, Q, cp, f2)
                     for x in xs])
    roots = []
...
def _find_boundary_entry_shock_on_profile(L, profile, cp, f2,
                                          q_so_min=0.02, q_so_max=0.16,
                                          xi_min=0.12, xi_max=0.23,
                                          rh_tol=0.02, welge_tol=0.012):
    """
    Find an entry shock P->Q from the So=0 boundary to the resolved
    early three-phase branch.
    """
    if abs(float(L[1])) > 1e-8:
        return None
    curve = np.asarray(profile, dtype=float)
    if curve.ndim != 2 or curve.shape[1] < 4 or len(curve) < 10:
        return None

    candidates = []
    for idx, row in enumerate(curve):
        Sw_q, So_q, Sg_q, xi_q = map(float, row[:4])
        if not (np.isfinite(Sw_q) and np.isfinite(So_q) and
                np.isfinite(Sg_q) and np.isfinite(xi_q)):
            continue
        if not (q_so_min <= So_q <= q_so_max and xi_min <= xi_q <= xi_max):
            continue
        try:
            l1_q, l2_q, _, _, disc_q = compute_eigenvalues(Sw_q, Sg_q, cp)
        except Exception:
            continue
        if not np.isfinite(disc_q) or disc_q < -1e-6:
            continue
        if abs(xi_q - l2_q) > max(abs(xi_q - l1_q), 0.025):
            continue

        Q = np.array([Sw_q, So_q, Sg_q])
        for Sw_P in _edge_rh_roots_for_Q(Q, L, cp, f2, n=260):
            P = np.array([Sw_P, 0.0, 1.0 - Sw_P])
            if P[2] <= Q[2] + 1e-5:
                continue
            sig, rh = compute_shock_speed(P, Q, cp, f2)
            if not np.isfinite(sig) or sig <= 0 or rh > rh_tol:
                continue
            bspd = _boundary_wave_speed(P[0], cp, f2)
            if not np.isfinite(bspd) or bspd >= sig:
                continue
            welge = abs(sig - l2_q)
            if welge > welge_tol:
                continue
            xi_match = abs(xi_q - l2_q)
            score = rh + welge + 0.2 * xi_match
            candidates.append((score, rh, welge, xi_match, idx, P, Q,
                               float(sig), float(l2_q), float(bspd),
                               float(xi_q)))

    if not candidates:
        print("      [entry-profile] no admissible P->Q entry shock found")
        return None
    candidates.sort(key=lambda x: x[0])
    score, rh, welge, xi_match, idx, P, Q, sig, lam_Q, bspd, xi_Q = candidates[0]
    print(f"      [entry-profile] FOUND: "
          f"P=({P[0]:.4f},{P[1]:.4f},{P[2]:.4f}) -> "
          f"Q=({Q[0]:.4f},{Q[1]:.4f},{Q[2]:.4f})")
    print(f"      [entry-profile]   sigma(P,Q)={sig:.4f}, "
          f"lambda2(Q)={lam_Q:.4f}, xi_Q={xi_Q:.4f}, "
          f"boundary_speed(P)={bspd:.4f}, RH={rh:.2e}, "
          f"Welge={welge:.2e}")
    return {
        'P': P, 'Q': Q, 'sig_PQ': sig, 'lam_Q': lam_Q,
        'rh_PQ': rh, 'welge_err': welge,
        'boundary_speed_P': bspd, 'idx_Q': int(idx),
        'xi_Q_num': xi_Q, 'score': float(score),
    }


def _finalize_boundary_entry_solution(L, R, M_fast, entry, rar_QR, cp, f2):
    """Package boundary rarefaction, entry shock, fast transition, terminal shock."""
    P = entry['P'].copy()
    Q = entry['Q'].copy()
    sig_PQ = float(entry['sig_PQ'])
    rar_QR = np.asarray(rar_QR, dtype=float)
    if rar_QR.ndim != 2 or rar_QR.shape[1] < 4 or len(rar_QR) == 0:
        rar_QR = np.array([[Q[0], Q[1], Q[2], entry['lam_Q']]])
    ...
    xi_b, Sw_b, Sg_b, So_b = xi_b[order], Sw_b[order], Sg_b[order], So_b[order]
    ...
    # Keep only a short constant-state tail after the terminal wave.  The old
    # hard lower bound of 60 stretched the R plateau far beyond any wave-speed
    # content in this case.
    xhi = float(np.nanmax(x_all)) + max(0.15 * np.ptp(x_all), 1.0)

    xi_parts, Sw_parts, So_parts, Sg_parts = [], [], [], []
    ...
    mask = rar_QR[:, 3] > xi_right
    _append(rar_QR[mask, 3], rar_QR[mask, 0], rar_QR[mask, 1], rar_QR[mask, 2])
    if xi_parts[-1] < xhi:
    ...
    fast_path = np.vstack([shock_line, rar_QR])

    return {
        'xi': xi, 'Sw': Sw, 'So': So, 'Sg': Sg, 'fw': fw, 'fg': fg,
        'slow_path': slow_path, 'fast_path': fast_path,
        'slow_type': 'boundary-rarefaction',
        'fast_type': 'entry-fast-transition-shock',
        'fast_has_terminal_shock': True,
        'fast_shock_sigma': float(np.nanmax(rar_QR[:, 3])),
        'L': L, 'M': M_fast, 'R': R,
        'P_entry': P, 'Q_entry': Q,
...
def _make_fast_composite_MR(M, R, cp, f2, rh_tol=0.05):
    """
    Build the composite fast wave M -> R as

        rar(M -> J2) + shock(J2 -> R),

    where J2 is the Welge tangent on the fast integral curve from M with
    sigma(J2, R) = lambda_2(J2).  Returns
    (fast_path, fast_xi, fast_Sw, fast_Sg, J2, sigma_J2R, lam2_J2)
    or seven Nones.
    """
    print("    Strategy fast-composite (M->R): rar + Welge shock ...")
    crv_p = integrate_rarefaction(M[0], M[2], 2, +1, cp, f2,
                                   Nmax=15000, h=0.0003)
    crv_m = integrate_rarefaction(M[0], M[2], 2, -1, cp, f2,
                                   Nmax=15000, h=0.0003)
    try:
        _, l2_M, _, _, _ = compute_eigenvalues(M[0], M[2], cp)
    except Exception:
        return (None,) * 7

    # Pick the branch whose lambda_2 INCREASES (toward R)
    crv_use = None
    for c in (crv_p, crv_m):
        if len(c) >= 5 and c[:, 3].max() > l2_M + 1e-3:
            crv_use = c
            break
    if crv_use is None:
        print("      [fastMR] no fast integral curve from M with lam2 > lam2(M)")
        return (None,) * 7

    # Keep only the lambda_2 >= lambda_2(M) portion, sorted ascending
    keep = crv_use[:, 3] >= l2_M - 1e-6
    crv_use = crv_use[keep]
    crv_use = crv_use[np.argsort(crv_use[:, 3])]
    if len(crv_use) < 5:
        return (None,) * 7

    print(f"      [fastMR] curve {len(crv_use)} pts  "
          f"lam2 in [{crv_use[0,3]:.4f}, {crv_use[-1,3]:.4f}]")

    residuals = np.full(len(crv_use), np.nan)
    sigs = np.full(len(crv_use), np.nan)
    rhs = np.full(len(crv_use), np.nan)
    for i, row in enumerate(crv_use):
        P = np.array([row[0], 1.0 - row[0] - row[2], row[2]])
        sig, rh = compute_shock_speed(P, R, cp, f2)
        if not np.isfinite(sig):
            continue
        residuals[i] = row[3] - sig
    ...
    # Welge sign-change search runs on ALL finite residuals (the residual
    # sigma(P,R) crosses lambda_2(P) once at the tangent regardless of rh).
    # Only AFTER the tangent is located do we check that rh is acceptable.
    valid = np.where(np.isfinite(residuals))[0]
    if len(valid) < 2:
        print("      [fastMR] no finite residuals")
        return (None,) * 7

    sign_change = None
    for k in range(len(valid) - 1):
        a, b = valid[k], valid[k + 1]
        if residuals[a] * residuals[b] <= 0:
            sign_change = (a, b)
            break

    if sign_change is None:
        i_W = int(valid[np.argmin(np.abs(residuals[valid]))])
        method = 'min_residual'
    else:
        a, b = sign_change
        i_W = a if abs(residuals[a]) <= abs(residuals[b]) else b
        method = 'sign_change'

    J2 = np.array([crv_use[i_W, 0],
                   1.0 - crv_use[i_W, 0] - crv_use[i_W, 2],
                   crv_use[i_W, 2]])
    sig_J2R = float(sigs[i_W]) if np.isfinite(sigs[i_W]) else float('nan')
    lam2_J2 = float(crv_use[i_W, 3])
    rh_J2R = float(rhs[i_W]) if np.isfinite(rhs[i_W]) else float('nan')

    if not np.isfinite(sig_J2R) or rh_J2R > rh_tol * 2:
        print(f"      [fastMR] best J2 has rh={rh_J2R:.2e} > tol; "
              f"accepting anyway (will be vetted upstream)")

    print(f"      [fastMR] J2 = ({J2[0]:.4f},{J2[1]:.4f},{J2[2]:.4f})  "
          f"lam2={lam2_J2:.4f}  sigma(J2,R)={sig_J2R:.4f}  "
          f"rh={rh_J2R:.2e}  ({method})")

    rar_seg = crv_use[:i_W + 1].copy()

    fast_Sw = np.concatenate([rar_seg[:, 0], [R[0]]])
    fast_Sg = np.concatenate([rar_seg[:, 2], [R[2]]])
    fast_xi = np.concatenate([rar_seg[:, 3], [sig_J2R]])
    fast_xi = np.maximum.accumulate(fast_xi)

    fast_path = np.column_stack([
        fast_Sw, 1.0 - fast_Sw - fast_Sg, fast_Sg, fast_xi
    ])
    return fast_path, fast_xi, fast_Sw, fast_Sg, J2, sig_J2R, lam2_J2


def _find_trans_shock_across_elliptic(L, M, R, cp, f2,
                                       score_tol=0.10, rh_tol=0.05):
    """
    Build the fast wave as

        L --R_2--> J --(transitional shock J->K, spans disc<0)--> K
            --R_2--> M --S_2--> R

    where J sits on the fast integral curve through L and K on the fast
    integral curve through M, both AT or just inside the elliptic-region
    boundary on their respective sides.  The pair (J, K) is selected by the
    doubly-characteristic / Liu condition

        sigma(J, K) = lambda_2(J) = lambda_2(K),    rh(J, K) ~ 0

    so that the trailing characteristic of rar(L->J) and the leading
    characteristic of rar(K->M) both match the shock speed.  This is the
    Isaacson-Marchesin-Plohr transitional shock structure.

    Returns (J, K, rar_LJ, rar_KM, sigma_JK, info) or six Nones.
    """
    print("    Strategy elliptic-cross: rar(L->J) + trans-shock(J->K) + "
          "rar(K->M) ...")

    # ---- L-side fast integral curve: walk until disc<0 (auto-stops there)
    crv_L_p = integrate_rarefaction(L[0], L[2], 2, +1, cp, f2,
                                     Nmax=15000, h=0.0003)
    crv_L_m = integrate_rarefaction(L[0], L[2], 2, -1, cp, f2,
                                     Nmax=15000, h=0.0003)
    try:
        _, l2_L, _, _, _ = compute_eigenvalues(L[0], L[2], cp)
    except Exception:
        return (None,) * 6

    crv_L_use = None
    for c in (crv_L_p, crv_L_m):
        if len(c) >= 5 and c[:, 3].max() > l2_L + 1e-3:
            crv_L_use = c
            break
    if crv_L_use is None:
        for c in (crv_L_p, crv_L_m):
            if len(c) >= 5:
                crv_L_use = c
                break
    if crv_L_use is None:
        print("      [trans] no usable fast curve from L")
        return (None,) * 6

    # ---- M-side fast integral curve: branch with lambda_2 < lambda_2(M)
    try:
        _, l2_M, _, _, _ = compute_eigenvalues(M[0], M[2], cp)
    except Exception:
        return (None,) * 6
    crv_M_p = integrate_rarefaction(M[0], M[2], 2, +1, cp, f2,
                                     Nmax=15000, h=0.0003)
    crv_M_m = integrate_rarefaction(M[0], M[2], 2, -1, cp, f2,
                                     Nmax=15000, h=0.0003)
    crv_M_use = None
    for c in (crv_M_p, crv_M_m):
        if len(c) >= 5 and c[:, 3].min() < l2_M - 1e-3:
            crv_M_use = c
            break
    if crv_M_use is None:
        print("      [trans] no fast curve branch from M with lam2 < lam2(M)")
        return (None,) * 6

    print(f"      [trans] L-curve raw {len(crv_L_use)} pts  "
          f"lam2 in [{crv_L_use[:,3].min():.4f},{crv_L_use[:,3].max():.4f}]")
    print(f"      [trans] M-curve raw {len(crv_M_use)} pts  "
          f"lam2 in [{crv_M_use[:,3].min():.4f},{crv_M_use[:,3].max():.4f}]")

    # ---- Manually trim each curve to its hyperbolic prefix (the integrator
    # check d < -1e-10 may not always trigger, especially near the elliptic
    # boundary where disc dips just barely below zero).
    eps_disc = 1e-5
    def hyperbolic_prefix(crv):
        for k in range(len(crv)):
            try:
                _, _, _, _, d = compute_eigenvalues(crv[k, 0], crv[k, 2], cp)
                if d < eps_disc:
                    return crv[:max(k, 1)]
            except Exception:
                return crv[:max(k, 1)]
    ...
    crv_L_hyp = hyperbolic_prefix(crv_L_use)
    crv_M_hyp = hyperbolic_prefix(crv_M_use)
    print(f"      [trans] L-curve hyperbolic {len(crv_L_hyp)} pts  "
          f"lam2 in [{crv_L_hyp[:,3].min():.4f},{crv_L_hyp[:,3].max():.4f}]")
    print(f"      [trans] M-curve hyperbolic {len(crv_M_hyp)} pts  "
          f"lam2 in [{crv_M_hyp[:,3].min():.4f},{crv_M_hyp[:,3].max():.4f}]")

    if len(crv_L_hyp) < 5 or len(crv_M_hyp) < 5:
        print("      [trans] hyperbolic prefix too short on one or both curves")
        return (None,) * 6

    # The (Sw,Sg) chord J->K should cross the elliptic region.  Sanity-check
    # that the two curves' boundary endpoints are on opposite sides of the
    # gap: distance(L-end, M-end) should be a meaningful fraction of the
    # elliptic disc diameter (>= 0.05 in Sw,Sg).
    end_dist = float(np.sqrt((crv_L_hyp[-1, 0] - crv_M_hyp[-1, 0])**2 +
                              (crv_L_hyp[-1, 2] - crv_M_hyp[-1, 2])**2))
    print(f"      [trans] dist(L-curve_end, M-curve_end) = {end_dist:.4f}")

    # ---- Search the last ~120 hyperbolic points (i.e. closest to the
    # elliptic boundary on each side) for an admissible (J, K) pair
    nL = min(len(crv_L_hyp), 120)
    nM = min(len(crv_M_hyp), 120)
    L_tail = crv_L_hyp[-nL:]
    M_tail = crv_M_hyp[-nM:]

    best = None
    for iJ, rowJ in enumerate(L_tail):
        J = np.array([rowJ[0], 1.0 - rowJ[0] - rowJ[2], rowJ[2]])
        lJ = float(rowJ[3])
        for iK, rowK in enumerate(M_tail):
            K = np.array([rowK[0], 1.0 - rowK[0] - rowK[2], rowK[2]])
            lK = float(rowK[3])
            sig, rh = compute_shock_speed(J, K, cp, f2)
            if not np.isfinite(sig) or rh > rh_tol:
                continue
            res_J = abs(sig - lJ)
            res_K = abs(sig - lK)
            score = res_J + res_K + 5.0 * rh
            cand = (score, J, K, iJ, iK, float(sig), float(rh), lJ, lK)
            if best is None or cand[0] < best[0]:
                best = cand

    if best is None:
        print(f"      [trans] no (J,K) with rh < {rh_tol}")
        return (None,) * 6

    score, J, K, iJ, iK, sig_JK, rh_JK, lJ, lK = best
    print(f"      [trans] best: J=({J[0]:.4f},{J[1]:.4f},{J[2]:.4f}) "
          f"lam2={lJ:.4f}")
    print(f"      [trans]       K=({K[0]:.4f},{K[1]:.4f},{K[2]:.4f}) "
          f"lam2={lK:.4f}")
    print(f"      [trans]       sigma(J,K)={sig_JK:.4f}  rh={rh_JK:.2e}  "
          f"score={score:.4f}")
    if score > score_tol:
        print(f"      [trans] score {score:.4f} > tol {score_tol}; "
              f"transitional shock not admissible")
        return (None,) * 6

    # ---- Build rarefaction segments along the HYPERBOLIC curves
    iJ_glob = len(crv_L_hyp) - nL + iJ
    iK_glob = len(crv_M_hyp) - nM + iK

    rar_LJ = crv_L_hyp[:iJ_glob + 1].copy()
    seg_M_to_K = crv_M_hyp[:iK_glob + 1].copy()
    rar_KM = seg_M_to_K[::-1]  # reverse so it goes K -> M

    # Sort by ascending lam_2 (rarefaction parameterization in xi)
    rar_LJ = rar_LJ[np.argsort(rar_LJ[:, 3])]
    rar_KM = rar_KM[np.argsort(rar_KM[:, 3])]
    # Shocks must come AFTER the trailing rarefaction characteristic
    if not (sig_JK > lJ - 1e-3 and sig_JK < lK + 1e-3):
        # Doubly-characteristic condition has some slack but check ordering
        pass

    info = {
        'sigma_JK': sig_JK, 'rh_JK': rh_JK,
        'lam2_J': lJ, 'lam2_K': lK,
        'rar_LJ_npts': int(len(rar_LJ)),
        'rar_KM_npts': int(len(rar_KM)),
        'score': float(score),
    }
    print(f"      [trans] rar(L->J): {info['rar_LJ_npts']} pts  "
          f"rar(K->M): {info['rar_KM_npts']} pts")
    return J, K, rar_LJ, rar_KM, sig_JK, info
...
def solve_S1R2(L, R, cp, f2, Npts=600):
    ...
    for i in range(1, len(rar2)):
        ...
        if So_m < SORG + max(_SO_REF, 0.01) or Sw_m < SWC + 0.001:
            counts['bounds'] += 1
            continue
...
def solve_R1R2(L, R, cp, f2, Npts=3000, max_pc=30, tol=1e-6):
    ...
    # Restrict to interior (So > SORG) to avoid degenerate boundary points
    slow_int = slow_crv[slow_crv[:, 1] > SORG + 0.005]
    fast_int = fast_crv[fast_crv[:, 1] > SORG + 0.005]
...
# =====================================================================
#  R1S1R2S2 solver  (Juanes & Patzek 2004, Appendix B.3)
# =====================================================================
def _r1s1_fit(um, L, r1_curve_forward, cp, f2):
    """Locate u*_1 on the R1(L) integral curve (sigma1 = nu1(u*_1) tangent
    + RH for u*_1 -> um).  Returns (residual, index, sigma1).  Restricted
    to the segment where nu1 is non-decreasing from L."""
    nu1_L = float(r1_curve_forward[0, 3])
    fw_m, _, fg_m = fractional_flow(um[0], um[2], cp, f2)
    best_res = np.inf; best_i = -1; best_sig = 0.0
    for i in range(len(r1_curve_forward)):
        pt = r1_curve_forward[i]
        if pt[3] < nu1_L - 1e-8:
            break
        Sw_s, Sg_s = float(pt[0]), float(pt[2])
        fw_s, _, fg_s = fractional_flow(Sw_s, Sg_s, cp, f2)
        sigma = float(pt[3])
        rw = (fw_m - fw_s) - sigma * (um[0] - Sw_s)
        rg = (fg_m - fg_s) - sigma * (um[2] - Sg_s)
        denom = max(abs(fw_m - fw_s), abs(fg_m - fg_s), 1e-12)
        res = np.hypot(rw, rg) / denom
        if res < best_res:
            best_res = res; best_i = i; best_sig = sigma
    return best_res, best_i, best_sig


def _r2s2_fit(um, R, cp, f2, Nmax=3000, h=0.0006):
    """Trace R2(um) forward, locate u*_2 satisfying sigma2 = nu2(u*_2) + RH
    for u*_2 -> R.  Returns (residual, u*_2, sigma2, r2_curve)."""
    r2 = integrate_rarefaction(um[0], um[2], 2, +1, cp, f2, Nmax=Nmax, h=h)
    if len(r2) < 3:
        return np.inf, None, 0.0, r2
    nu2_um = float(r2[0, 3])
    fw_R, _, fg_R = fractional_flow(R[0], R[2], cp, f2)
    best_res = np.inf; best_i = -1; best_sig = 0.0
    for i in range(len(r2)):
        ...
        fw_s, _, fg_s = fractional_flow(Sw_s, Sg_s, cp, f2)
        sigma = float(pt[3])
        rw = (fw_R - fw_s) - sigma * (R[0] - Sw_s)
        rg = (fg_R - fg_s) - sigma * (R[2] - Sg_s)
        denom = max(abs(fw_R - fw_s), abs(fg_R - fg_s), 1e-12)
        res = np.hypot(rw, rg) / denom
        if res < best_res:
            best_res = res; best_i = i; best_sig = sigma
    u_star_2 = r2[best_i, :3].copy() if best_i >= 0 else None
    return best_res, u_star_2, best_sig, r2


def solve_R1S1R2S2(L, R, cp, f2,
                   min_r1_len=0.05, min_s1_len=0.03, min_lm=0.08):
    """Juanes-Patzek Appendix B.3: L --R1-> u*_1 --S1-> M --R2-> u*_2 --S2-> R.

    Penalizes degenerate solutions where the R1 or S1 segments collapse to
    a point.  Returns (M, sigma1, sigma2, ok, u*_1, u*_2).
    """
    from scipy.optimize import minimize

    r1_fwd = integrate_rarefaction(L[0], L[2], 1, +1, cp, f2,
                                    Nmax=4000, h=0.0006)
    if len(r1_fwd) < 5:
        r1_fwd = integrate_rarefaction(L[0], L[2], 1, -1, cp, f2,
                                        Nmax=4000, h=0.0006)
    if len(r1_fwd) < 5:
        print("      [R1S1R2S2] R1(L) curve too short")
        return None, np.nan, np.nan, False, None, None

    def _d(a_sw, a_sg, b_sw, b_sg):
        return np.hypot(a_sw - b_sw, a_sg - b_sg)

    def objective(x):
        Sw_m, Sg_m = x
        if Sw_m < 1e-4 or Sg_m < 1e-4 or Sw_m + Sg_m > 1 - 1e-4:
            return 1e4
        um = np.array([Sw_m, 1.0 - Sw_m - Sg_m, Sg_m])
        r_slow, i1, _ = _r1s1_fit(um, L, r1_fwd, cp, f2)
        r_fast, _, _, _ = _r2s2_fit(um, R, cp, f2)
        # Anti-degeneracy: penalize solutions where |u*_1 - L| or |um - u*_1|
        # is too small (these make R1 or S1 collapse, which is the trivial
        # zero-residual minimum).
        penalty = 0.0
        if i1 >= 0:
            u1_sw = float(r1_fwd[i1, 0]); u1_sg = float(r1_fwd[i1, 2])
            r1_len = _d(u1_sw, u1_sg, L[0], L[2])
            s1_len = _d(Sw_m, Sg_m, u1_sw, u1_sg)
            if r1_len < min_r1_len:
                penalty += 10.0 * (min_r1_len - r1_len)
            if s1_len < min_s1_len:
                penalty += 10.0 * (min_s1_len - s1_len)
        lm_len = _d(Sw_m, Sg_m, L[0], L[2])
        if lm_len < min_lm:
            penalty += 20.0 * (min_lm - lm_len)
        return r_slow + r_fast + penalty

    # Seed from a wide spread of interior guesses
    x0_list = [
        np.array([0.5 * (L[0] + R[0]), 0.5 * (L[2] + R[2])]),
        np.array([0.3, 0.4]), np.array([0.35, 0.35]),
        np.array([0.45, 0.25]), np.array([0.4, 0.3]),
        np.array([0.55, 0.15]), np.array([0.3, 0.5]),
        np.array([0.6, 0.2]), np.array([0.5, 0.25]),
    ]
    ...
    for x0 in x0_list:
        try:
            ...
            if opt.fun < best_val:
                best_val = opt.fun; best = opt
        except Exception:
            continue

    if best is None:
        print("      [R1S1R2S2] optimizer failed")
        return None, np.nan, np.nan, False, None, None

    Sw_m, Sg_m = best.x
    um = np.array([Sw_m, 1.0 - Sw_m - Sg_m, Sg_m])
    r_slow, i1, sigma1 = _r1s1_fit(um, L, r1_fwd, cp, f2)
    r_fast, u_star_2, sigma2, _ = _r2s2_fit(um, R, cp, f2)
    u_star_1 = r1_fwd[i1, :3].copy() if i1 >= 0 else None

    print(f"      [R1S1R2S2] M=({um[0]:.4f},{um[1]:.4f},{um[2]:.4f})  "
          f"objective(incl. penalty)={best_val:.3e}")
    if u_star_1 is not None:
        print(f"      [R1S1R2S2] u*_1=({u_star_1[0]:.4f},{u_star_1[1]:.4f},"
              f"{u_star_1[2]:.4f})  sigma1={sigma1:.4f}  "
              f"slow_res={r_slow:.3e}")
    if u_star_2 is not None:
        print(f"      [R1S1R2S2] u*_2=({u_star_2[0]:.4f},{u_star_2[1]:.4f},"
              f"{u_star_2[2]:.4f})  sigma2={sigma2:.4f}  "
              f"fast_res={r_fast:.3e}")

    # Admissibility: RH residuals small, wave-speed ordering, non-degenerate
    lm_len = _d(Sw_m, Sg_m, L[0], L[2])
    if u_star_1 is not None:
        r1_len = _d(float(u_star_1[0]), float(u_star_1[2]), L[0], L[2])
        s1_len = _d(Sw_m, Sg_m, float(u_star_1[0]), float(u_star_1[2]))
    else:
        r1_len = 0.0; s1_len = 0.0
    ok = (r_slow + r_fast < 0.05) and (sigma1 < sigma2 + 1e-3) \
        and (lm_len > min_lm) and (r1_len > min_r1_len) \
        and (s1_len > min_s1_len)
    if not ok:
        print(f"      [R1S1R2S2] rejected: r_slow+r_fast={r_slow+r_fast:.3e}, "
              f"|M-L|={lm_len:.4f} (>{min_lm}), R1 |dS|={r1_len:.4f} "
              f"(>{min_r1_len}), S1 |dS|={s1_len:.4f} (>{min_s1_len})")
    return um, sigma1, sigma2, ok, u_star_1, u_star_2
...
def find_intermediate_state(L, R, cp, f2):
    ...
    find_intermediate_state._rsrs_candidate = None

    # --- Strategy R1S1R2S2 (Juanes-Patzek Appendix B.3, try first) ---
    # Explicitly finds u*_1 on V_1 and u*_2 on V_2 via Welge tangency and
    # RH matching.  Accepted only when the R1 and S1 segments are both
    # non-degenerate (|u*_1 - L| and |M - u*_1| above thresholds), so a
    # collapsed u*_1 = L = M trivial minimum is rejected.
    print("    Strategy R1S1R2S2: composite slow + composite fast ...")
    M_rsrs, s1_rsrs, s2_rsrs, ok_rsrs, us1, us2 = solve_R1S1R2S2(L, R, cp, f2)
    if M_rsrs is not None and us2 is not None and np.isfinite(s2_rsrs):
        # Keep the candidate even when the whole R1S1R2S2 structure is
        # rejected.  In weakly-water-wet cases the slow side can collapse,
        # while the terminal R2/S2 Welge point remains an excellent closure
        # for the final fast shock.
        find_intermediate_state._rsrs_candidate = {
            'M': M_rsrs.copy(), 'sigma1': float(s1_rsrs),
            'sigma2': float(s2_rsrs),
            'u_star_1': None if us1 is None else us1.copy(),
            'u_star_2': us2.copy(),
            'ok': bool(ok_rsrs),
        }
    if ok_rsrs:
        M_rsrs[1] = 1.0 - M_rsrs[0] - M_rsrs[2]
        find_intermediate_state._wave_type = 'R1S1R2S2'
        find_intermediate_state._sigma1 = s1_rsrs
        find_intermediate_state._sigma2 = s2_rsrs
        find_intermediate_state._u_star_1 = us1
        find_intermediate_state._u_star_2 = us2
        print(f"  M = ({M_rsrs[0]:.4f}, {M_rsrs[1]:.4f}, {M_rsrs[2]:.4f})"
              f"  [R1S1R2S2]")
        return M_rsrs
    ...
    # --- Strategy 0b: fast-only composite (try early) ---
    # When L sits on the So=SORG edge and our model has the slow eigenvector
    # tangent to the edge, the slow family cannot carry the Riemann solution
    # into the interior -- the whole wave is a single fast-family composite
    # S_L R_L (or just R) that enters the interior from L.  Try this
    # structure FIRST so we do not bury it under Strategy 1c's transitional
    # shock fallback.
    L_on_edge = (abs(L[1] - SORG) <= 1e-4)
    if L_on_edge:
        # Check if L->R chord crosses the elliptic region.
        # If so, fast-only is wrong — need transitional shock.
        ...
        if _crosses_elliptic:
            ...
            ok_fo, tail_fo, peak_fo, rar_seg_fo = _try_fast_only_solution(
                L, R, cp, f2)
            # Direction filter: the fast-only tail is just the closest
            # point of the fast rar(R) curve to L.  When L sits deep in
            # the interior of the So=0 edge (e.g. L=(0.5,0,0.5)), the
            # geometrically-closest point can be on the wrong side of L
            # in (Sw, Sg) — it would correspond to a slow shock going
            # backwards.  Only accept the fast-only short-circuit when the
            # tail lies between L and R in the dominant component.
            dSw_LR = R[0] - L[0]
            dSg_LR = R[2] - L[2]
            tail_ok = True
            if ok_fo:
                bad_sw = dSw_LR * (tail_fo[0] - L[0]) < -1e-3
                bad_sg = dSg_LR * (tail_fo[2] - L[2]) < -1e-3
                if bad_sw or bad_sg:
                    tail_ok = False
                    print(f"    [fast-only] tail=({tail_fo[0]:.4f},"
                          f"{tail_fo[1]:.4f},{tail_fo[2]:.4f}) moves wrong "
                          f"direction vs L->R (bad_Sw={bad_sw}, "
                          f"bad_Sg={bad_sg}); skipping fast-only")
            if ok_fo and tail_ok:
                M_fo = tail_fo.copy()
    ...
    if M is None:
        ...
        if len(slow_crv) > 0 and len(fast_crv) > 0:
            ...
            _so_floor = SORG + max(_SO_REF_1b, 0.01)
...
def construct_solution(L, R, M, cp, f2):
    ...
    if fast_only:
        sol = _construct_fast_only(L, R, M, cp, f2)
    else:
        sol = _construct_slow_fast(L, R, M, cp, f2)

    # -- Diagnostic: wave-speed variation along each path segment ----------
    def _dump(path_arr, name):
        ...
        if n > 14:
            idxs = sorted(set([0]
                              + list(np.linspace(1, n - 2, 10).astype(int))
                              + [n - 2, n - 1]))
        else:
            idxs = list(range(n))
        print(f"  --- {name} speed table ---")
        print("     i    Sw     So     Sg       xi       lam1      lam2")
        for i in idxs:
            pt = path_arr[i]
            try:
                l1, l2, _, _, _ = compute_eigenvalues(float(pt[0]),
                                                      float(pt[2]), cp)
            except Exception:
                l1, l2 = np.nan, np.nan
            print(f"  {i:4d}  {pt[0]:.4f}  {1 - pt[0] - pt[2]:.4f}  "
                  f"{pt[2]:.4f}  {pt[3]:8.4f}  {l1:8.4f}  {l2:8.4f}")

    try:
        M_diag = sol.get('M', M)
        l1_L, l2_L, _, _, _ = compute_eigenvalues(L[0], L[2], cp)
        l1_M, l2_M, _, _, _ = compute_eigenvalues(M_diag[0], M_diag[2], cp)
        l1_R, l2_R, _, _, _ = compute_eigenvalues(R[0], R[2], cp)
        s_LM, _ = compute_shock_speed(L, M_diag, cp, f2)
        s_MR, _ = compute_shock_speed(M_diag, R, cp, f2)
        _dump(sol.get('slow_path'),
              f"Slow wave L->M  type={sol.get('slow_type')}")
        print(f"    endpoints: lam1(L)={l1_L:.4f}  lam1(M)={l1_M:.4f}  "
              f"sigma(L,M)={s_LM:.4f}")
        _dump(sol.get('fast_path'),
              f"Fast wave M->R  type={sol.get('fast_type')}")
        print(f"    endpoints: lam2(M)={l2_M:.4f}  lam2(R)={l2_R:.4f}  "
              f"sigma(M,R)={s_MR:.4f}")
    except Exception as e:
        print(f"    (speed-table diagnostic failed: {e})")
...
def _build_analytical_fast_composite(L, M, cp, f2):
    """Build the analytical L -> M fast-composite path when L and M lie on
    the same fast (lambda_2) integral curve and lambda_2 is non-monotone
    along it (the curve folds at the V_2 inflection locus).

    Structure (Liu / Oleinik construction at the inflection):

        L  =[zero-strength jump]=>  L_eq
            --R_2 (lambda_2 increasing)--> u_a   (state on V_2, lambda_2 local max)
            --S_2 interior shock at sigma=lambda_2(u_a)--> u_b
            --R_2 (lambda_2 increasing)--> M

    Returns dict with L_eq, u_a, u_b, sigma_int, rar1 (L_eq->u_a),
    rar2 (u_b->M), or None when geometry doesn't match.
    """
    try:
        _, lam2_M, _, _, _ = compute_eigenvalues(M[0], M[2], cp)
    except Exception:
    ...
    branches = []
    for direction in (-1, +1):
        cb = integrate_rarefaction(M[0], M[2], 2, direction, cp, f2,
                                    Nmax=12000, h=0.0003)
        if len(cb) < 20:
            continue
        d2L_b = (cb[:, 0] - L[0])**2 + (cb[:, 2] - L[2])**2
        iLb = int(np.argmin(d2L_b))
        if iLb < 5:
            continue
        dist_Lb = float(np.sqrt(d2L_b[iLb]))
        branches.append((dist_Lb, iLb, cb))
        print(f"      [analytical-fast-composite] dir={direction:+d}: "
              f"{len(cb)} pts, lam2 in [{cb[:,3].min():.4f},"
              f"{cb[:,3].max():.4f}], closest to L at i={iLb} "
              f"|P-L|={dist_Lb:.4e}")

    if not branches:
        print(f"      [analytical-fast-composite] no usable branch from M")
        return None

    branches.sort(key=lambda t: t[0])
    dist_L, iL, crv = branches[0]
    if dist_L > 0.05:
        print(f"      [analytical-fast-composite] best |P-L|={dist_L:.4e} "
              f"> 0.05; curve does not reach L")
        return None
    ...
    for i in range(len(diffs) - 1):
        if diffs[i] * diffs[i + 1] < 0:
            kind = 'max' if diffs[i] > 0 else 'min'
            extrema.append((i + 1, kind))
    print(f"      [analytical-fast-composite] extrema on M->L: "
          f"{len(extrema)} {[(i, k) for i,k in extrema[:6]]}")
    i_min = next((i for i, k in extrema if k == 'min'), None)
    i_max = next((i for i, k in extrema if k == 'max'), None)
    if i_min is None or i_max is None or not (i_min < i_max):
        print(f"      [analytical-fast-composite] no usable bump "
              f"(i_min={i_min}, i_max={i_max})")
        return None

    sigma_int = float(lam[i_max])
    print(f"      [analytical-fast-composite] M->L curve: {len(seg)} pts; "
          f"lam2 in [{lam.min():.4f},{lam.max():.4f}]")
    print(f"      [analytical-fast-composite] V_2 bump: min i={i_min} "
          f"(lam2={lam[i_min]:.4f}), max i={i_max} (lam2={lam[i_max]:.4f})")

    u_a = seg[i_max, :3].copy()
    u_a[1] = 1.0 - u_a[0] - u_a[2]

    Mside = seg[:i_min + 1]
    cross_i = None
    for i in range(len(Mside) - 1):
        if (Mside[i, 3] >= sigma_int and Mside[i + 1, 3] < sigma_int):
            cross_i = i
            break
    if cross_i is None:
        return None
    a, b = Mside[cross_i, 3], Mside[cross_i + 1, 3]
    t = (a - sigma_int) / (a - b) if abs(a - b) > 1e-12 else 0.5
    u_b = np.array([
        Mside[cross_i, 0] + t * (Mside[cross_i + 1, 0] - Mside[cross_i, 0]),
        0.0,
        Mside[cross_i, 2] + t * (Mside[cross_i + 1, 2] - Mside[cross_i, 2]),
    ])
    u_b[1] = 1.0 - u_b[0] - u_b[2]

    sig_ab, rh_ab = compute_shock_speed(u_a, u_b, cp, f2)
    if not np.isfinite(sig_ab):
        return None
    welge_err = abs(sig_ab - sigma_int)
    print(f"      [analytical-fast-composite] u_a=({u_a[0]:.4f},"
          f"{u_a[1]:.4f},{u_a[2]:.4f})  u_b=({u_b[0]:.4f},{u_b[1]:.4f},"
          f"{u_b[2]:.4f})")
    print(f"      [analytical-fast-composite] sigma_int={sigma_int:.4f}  "
          f"sigma(u_a,u_b)={sig_ab:.4f}  |welge_err|={welge_err:.4f}  "
          f"rh={rh_ab:.2e}")

    if rh_ab > 0.05:
        print(f"      [analytical-fast-composite] RH residual too large; "
              f"abort")
        return None
    ...
    rar1 = seg_LM[:j_a + 1].copy()
    rar2 = seg_LM[j_b_hi:].copy()
    rar2[0, 0] = u_b[0]; rar2[0, 1] = u_b[1]
    rar2[0, 2] = u_b[2]; rar2[0, 3] = sigma_int

    def _strict_monotone(arr):
        if len(arr) < 2:
            return arr
        keep = [0]
        last_x = arr[0, 3]
        for i in range(1, len(arr)):
            if arr[i, 3] > last_x + 1e-9:
                keep.append(i)
                last_x = arr[i, 3]
        return arr[keep]

    rar1 = _strict_monotone(rar1)
    rar2 = _strict_monotone(rar2)
    if len(rar1) < 2 or len(rar2) < 2:
        return None

    rar1[-1, 0] = u_a[0]; rar1[-1, 1] = u_a[1]
    rar1[-1, 2] = u_a[2]; rar1[-1, 3] = sigma_int

    L_eq = rar1[0, :3].copy()
    L_eq[1] = 1.0 - L_eq[0] - L_eq[2]
    return {
        'L_eq': L_eq, 'u_a': u_a, 'u_b': u_b,
        'sigma_int': sigma_int, 'sig_ab': sig_ab, 'rh_ab': rh_ab,
        'rar1': rar1, 'rar2': rar2,
        'lam2_M': float(seg[0, 3]),
    }


def _fast_composite_from_numerical(L, R, M, cp, f2):
    """Numerical-profile-based fast-composite construction (fallback)."""
    ...
    if iL_end < 1:
        iL_end = 1
    d2R = (Sw_n - R[0])**2 + (Sg_n - R[2])**2
    close_R = d2R < 0.01**2
    iR_start = (int(np.argmax(close_R))
                if close_R.any() else len(xi_n) - 1)
    if iR_start <= iL_end + 10:
        iR_start = len(xi_n) - 1

    sig_term, _ = compute_shock_speed(M, R, cp, f2)
    sig_term = float(sig_term)
    xi_max_num = float(xi_n[iR_start])
    if not np.isfinite(sig_term) or sig_term <= xi_max_num:
        sig_term = xi_max_num + max(0.5, 0.1 * xi_max_num)
    seg_Sw = np.concatenate([[L[0]], Sw_n[iL_end:iR_start + 1], [R[0]]])
    seg_Sg = np.concatenate([[L[2]], Sg_n[iL_end:iR_start + 1], [R[2]]])
    seg_xi = np.concatenate([[xi_n[iL_end]],
                              xi_n[iL_end:iR_start + 1], [sig_term]])
    xi_u, idx_u = np.unique(seg_xi, return_index=True)
    Sw_u = seg_Sw[idx_u]; Sg_u = seg_Sg[idx_u]

    slow_xi_val = float(xi_u[0])
    slow_xi = np.array([slow_xi_val, slow_xi_val])
    slow_Sw = np.array([L[0], L[0]]); slow_Sg = np.array([L[2], L[2]])
    slow_path = np.array([
        [L[0], L[1], L[2], slow_xi_val],
        [L[0], L[1], L[2], slow_xi_val],
    ])
    slow_type = 'zero-strength'
    fast_xi, fast_Sw, fast_Sg = xi_u, Sw_u, Sg_u
    fast_path = np.column_stack([Sw_u, 1.0 - Sw_u - Sg_u, Sg_u, xi_u])
...
def _construct_slow_fast(L, R, M, cp, f2):
    ...
    # ----- R1S1R2S2 composite-composite (Juanes-Patzek Appendix B.3) -----
    # slow: L -(R1 on integral curve)-> u*_1 -(S1 at sigma1)-> M
    # fast: M -(R2 on integral curve)-> u*_2 -(S2 at sigma2)-> R
    if wave_type_hint_top == 'R1S1R2S2':
        us1 = find_intermediate_state._u_star_1
        us2 = find_intermediate_state._u_star_2
        sigma1 = float(find_intermediate_state._sigma1)
        sigma2 = float(find_intermediate_state._sigma2)
        print(f"    [R1S1R2S2] u*_1=({us1[0]:.4f},{us1[1]:.4f},{us1[2]:.4f})"
              f"  u*_2=({us2[0]:.4f},{us2[1]:.4f},{us2[2]:.4f})")

        # ---- Slow channel: R1 on slow integral curve L -> u*_1, then S1 ----
        r1_fwd = integrate_rarefaction(L[0], L[2], 1, +1, cp, f2,
                                        Nmax=4000, h=0.0006)
        if len(r1_fwd) < 5:
            r1_fwd = integrate_rarefaction(L[0], L[2], 1, -1, cp, f2,
                                            Nmax=4000, h=0.0006)
        d2_u1 = (r1_fwd[:, 0] - us1[0])**2 + (r1_fwd[:, 2] - us1[2])**2
        i_u1 = int(np.argmin(d2_u1))
        r1_seg = r1_fwd[:i_u1 + 1]
        # Enforce lam1 monotone-increasing
        if len(r1_seg) >= 2:
            r1_lam = np.maximum.accumulate(r1_seg[:, 3])
            r1_seg = r1_seg.copy()
            r1_seg[:, 3] = r1_lam

        slow_xi_rar = r1_seg[:, 3].copy()
        slow_Sw_rar = r1_seg[:, 0].copy()
        slow_Sg_rar = r1_seg[:, 2].copy()
        xi_u1, idx_u1 = np.unique(slow_xi_rar, return_index=True)
        slow_Sw_u = slow_Sw_rar[idx_u1]
        slow_Sg_u = slow_Sg_rar[idx_u1]

        slow_xi = np.append(xi_u1, sigma1)
        slow_Sw = np.append(slow_Sw_u, M[0])
        slow_Sg = np.append(slow_Sg_u, M[2])
        slow_path = np.vstack([
            np.column_stack([slow_Sw_u, 1.0 - slow_Sw_u - slow_Sg_u,
                              slow_Sg_u, xi_u1]),
            np.array([[M[0], M[1], M[2], sigma1]]),
        ])
        slow_type = 'composite'
        print(f"    [R1S1R2S2 slow] R1 {len(slow_Sw_u)} pts, "
              f"S1 sigma={sigma1:.4f}")

        # ---- Fast channel: R2 on fast integral curve M -> u*_2, then S2 ----
        r2_fwd = integrate_rarefaction(M[0], M[2], 2, +1, cp, f2,
                                        Nmax=4000, h=0.0006)
        d2_u2 = (r2_fwd[:, 0] - us2[0])**2 + (r2_fwd[:, 2] - us2[2])**2
        i_u2 = int(np.argmin(d2_u2))
        r2_seg = r2_fwd[:i_u2 + 1]
        if len(r2_seg) >= 2:
            r2_lam = np.maximum.accumulate(r2_seg[:, 3])
            r2_seg = r2_seg.copy()
            r2_seg[:, 3] = r2_lam

        fast_xi_rar = r2_seg[:, 3].copy()
        fast_Sw_rar = r2_seg[:, 0].copy()
        fast_Sg_rar = r2_seg[:, 2].copy()
        xi_u2, idx_u2 = np.unique(fast_xi_rar, return_index=True)
        fast_Sw_u = fast_Sw_rar[idx_u2]
        fast_Sg_u = fast_Sg_rar[idx_u2]

        fast_xi = np.append(xi_u2, sigma2)
        fast_Sw = np.append(fast_Sw_u, R[0])
        fast_Sg = np.append(fast_Sg_u, R[2])
        fast_path = np.vstack([
            np.column_stack([fast_Sw_u, 1.0 - fast_Sw_u - fast_Sg_u,
                              fast_Sg_u, xi_u2]),
            np.array([[R[0], R[1], R[2], sigma2]]),
        ])
        fast_type = 'composite'
        print(f"    [R1S1R2S2 fast] R2 {len(fast_Sw_u)} pts, "
              f"S2 sigma={sigma2:.4f}")
    ...
    if wave_type_hint_top == 'S1S2_trans':
        ...
        if H is None:
            print(f"    [S1S2_trans] fast_composite not applicable; "
                  f"using numerical L->M shape + fast shock M->R")
            # Keep wave_type_hint_top = 'S1S2_trans' so the numerical
            # extraction branch below executes (it is gated on the same
            # hint).  Previously this line set the hint to S1S2_trans_hyp,
            # which made the numerical-extraction fall-through unreachable
            # and silently degraded the solution to a trivial shock+shock.
    if wave_type_hint_top == 'S1S2_trans':
        ...
        if H_ok:
            ...
            # ---- PRIMARY: numerical-driven fast composite with a real
            # transitional shock across the elliptic region.  Wave structure:
            #     L --R_2 (along numerical fast curve)--> A
            #       --(transitional shock A->B, crosses disc<0 region)--> B
            #       --R_2 (along numerical fast curve)--> J2
            #       --S_2 (Lax-2 shock J2->R)--> R
            # Slow channel = zero-strength.  The whole solution lives in
            # the fast family, with a non-classical interior shock bridging
            # the elliptic gap that the inviscid integral curve cannot
            # cross.  Geometry is taken from the numerical profile so the
            # analytical and numerical (Sw,Sg) paths agree by construction.
            from numerical_solver import run_numerical
            num_p = run_numerical(L, R, cp, Nx=3000, t_final=0.4, quiet=True)
            xi_n = num_p['xi']
            Sw_n = num_p['Sw']
            Sg_n = num_p['Sg']

            # Trim L plateau and R plateau
            d2L = (Sw_n - L[0])**2 + (Sg_n - L[2])**2
            off_L = d2L > 0.005**2
            iL_end = int(np.argmax(off_L)) if off_L.any() else 1
            iL_end = max(iL_end, 1)
            ...
            # Locate the contiguous elliptic run (disc<0) along the active
            # window
            disc_n = np.full(iR_start - iL_end + 1, np.nan)
            for i, k in enumerate(range(iL_end, iR_start + 1)):
                try:
                    _, _, _, _, d = compute_eigenvalues(Sw_n[k], Sg_n[k], cp)
                    disc_n[i] = d
                except Exception:
                    pass
            ell_mask = np.isfinite(disc_n) & (disc_n < 0)
            ell_runs = []
            in_run = False
            a0 = None
            for i, e in enumerate(ell_mask):
                if e and not in_run:
                    in_run = True
                    a0 = i
                elif (not e) and in_run:
                    ell_runs.append((a0, i - 1))
                    in_run = False
            if in_run:
                ell_runs.append((a0, len(ell_mask) - 1))

            elliptic_present = len(ell_runs) > 0
            print(f"    [num-fast-trans] active window {iL_end}..{iR_start}, "
                  f"elliptic runs: {len(ell_runs)}")

            if elliptic_present:
                # Largest elliptic run
                ell_runs.sort(key=lambda r: r[1] - r[0], reverse=True)
                a_loc, b_loc = ell_runs[0]
                iA = iL_end + a_loc        # first elliptic sample (global idx)
                iB = iL_end + b_loc        # last elliptic sample
                # Snap A to the LAST hyperbolic sample BEFORE the gap, B to
                # the FIRST hyperbolic sample AFTER.
                iA = max(iA - 1, iL_end)
                iB = min(iB + 1, iR_start)
                A = np.array([Sw_n[iA], 1.0 - Sw_n[iA] - Sg_n[iA], Sg_n[iA]])
                B = np.array([Sw_n[iB], 1.0 - Sw_n[iB] - Sg_n[iB], Sg_n[iB]])
                sig_AB, rh_AB = compute_shock_speed(A, B, cp, f2)
                print(f"    [num-fast-trans] elliptic gap idx [{iA},{iB}]")
                print(f"    [num-fast-trans] A = ({A[0]:.4f},{A[1]:.4f},"
                      f"{A[2]:.4f})  xi_n={xi_n[iA]:.4f}")
                print(f"    [num-fast-trans] B = ({B[0]:.4f},{B[1]:.4f},"
                      f"{B[2]:.4f})  xi_n={xi_n[iB]:.4f}")
                print(f"    [num-fast-trans] chord shock A->B: sigma="
                      f"{sig_AB:.4f}  rh={rh_AB:.2e}")

                # Improve (A, B) by scanning a few neighbouring samples for
                # the lowest |rh| chord (RH-admissibility refinement).
                best_AB = (rh_AB, iA, iB, sig_AB, A.copy(), B.copy())
                for da in range(-3, 4):
                    for db in range(-3, 4):
                        ia = iA + da
                        ib = iB + db
                        if ia < iL_end or ib > iR_start:
                            continue
                        if ia >= ib:
                            continue
                        Aa = np.array([Sw_n[ia], 1.0 - Sw_n[ia] - Sg_n[ia],
                                       Sg_n[ia]])
                        Bb = np.array([Sw_n[ib], 1.0 - Sw_n[ib] - Sg_n[ib],
                                       Sg_n[ib]])
                        sig_ab, rh_ab = compute_shock_speed(Aa, Bb, cp, f2)
                        if not np.isfinite(sig_ab):
                            continue
                        if rh_ab < best_AB[0]:
                            best_AB = (rh_ab, ia, ib, float(sig_ab),
                                       Aa.copy(), Bb.copy())
                rh_AB, iA, iB, sig_AB, A, B = best_AB
                print(f"    [num-fast-trans] refined A->B (rh-best): "
                      f"sigma={sig_AB:.4f}  rh={rh_AB:.2e}  "
                      f"idx [{iA},{iB}]")

                # The numerical profile shows two distinct families:
                #   L -> A: boundary/slow rarefaction, xi ~ lambda_1
                #   A -> B: transitional shock across the elliptic gap
                #   B -> R: fast rarefaction followed by a terminal S_2 shock
                #
                # The old fallback put every segment into the fast family and
                # then estimated the final front from the short numerical
                # probe.  That made the analytical path look "fast-only" and
                # truncated the terminal shock speed.  Use B as the physical
                # intermediate state and close B -> R with the analytical
                # fast Welge construction.
                M_legacy = M.copy()
                M_trans = B.copy()

                # Build slow/transition path:
                #   rarefaction samples from L to A, then explicit A->B shock.
                pre_Sw = np.concatenate([[L[0]], Sw_n[iL_end:iA + 1]])
                pre_Sg = np.concatenate([[L[2]], Sg_n[iL_end:iA + 1]])
                pre_xi = np.concatenate([[xi_n[iL_end]],
                                          xi_n[iL_end:iA + 1]])
                sig_AB_eff = max(float(sig_AB), float(pre_xi[-1]) + 1e-3)
                slow_Sw_arr = np.concatenate([pre_Sw, [A[0], B[0]]])
                slow_Sg_arr = np.concatenate([pre_Sg, [A[2], B[2]]])
                slow_xi_arr = np.concatenate([pre_xi, [sig_AB_eff, sig_AB_eff]])
                slow_xi_arr = np.maximum.accumulate(slow_xi_arr)
                slow_path = np.column_stack([
                    slow_Sw_arr, 1.0 - slow_Sw_arr - slow_Sg_arr,
                    slow_Sg_arr, slow_xi_arr
                ])
                slow_xi, slow_Sw, slow_Sg = slow_xi_arr, slow_Sw_arr, slow_Sg_arr
                slow_type = 'slow-transition'

                fast_pack = _make_fast_composite_MR(M_trans, R, cp, f2,
                                                    rh_tol=0.08)
                if fast_pack[0] is not None:
                    (fast_path, fast_xi, fast_Sw, fast_Sg,
                     J2, sig_J2R, lam2_J2) = fast_pack
                    fast_type = 'fast-rarefaction-shock'
                    print(f"    [num-transition-split] fast closure: "
                          f"B->J2 rarefaction + J2->R shock; "
                          f"J2=({J2[0]:.4f},{J2[1]:.4f},{J2[2]:.4f}) "
                          f"sigma={sig_J2R:.4f}")
                else:
                    # B sits essentially on the umbilic/elliptic boundary, so
                    # direct fast-curve integration can stop immediately.  Use
                    # the numerical profile only until it is safely inside the
                    # fast family, then close the tail analytically by the
                    # R2/S2 Welge point J2 -> R.
                    print("    [num-transition-split] analytical B->R fast "
                          "closure failed at B; seeding from post-B fast tail")

                    seeded = None
                    seed_start = None
                    seed_candidates = []
                    lam_seed_min = max(float(sig_AB_eff) + 0.15, 0.50)
                    for k in range(iB + 1, iR_start + 1):
                        try:
                            l1_k, l2_k, _, _, d_k = compute_eigenvalues(
                                float(Sw_n[k]), float(Sg_n[k]), cp)
                        except Exception:
                            continue
                        if (np.isfinite(d_k) and d_k > 1e-5 and
                                l2_k > lam_seed_min and
                                (l2_k - l1_k) > 0.15):
                            seed_start = k
                            break
                    if seed_start is not None:
                        # Try a few increasingly downstream seeds.  Very near
                        # the umbilic the fast curve can still be too short;
                        # a slightly later state gives the same R2 branch.
                        offsets = [0, 20, 50, 100, 200, 400]
                        seed_candidates = [min(seed_start + off, iR_start)
                                           for off in offsets]

                    for k_seed in seed_candidates:
                        seed_state = np.array([
                            Sw_n[k_seed],
                            1.0 - Sw_n[k_seed] - Sg_n[k_seed],
                            Sg_n[k_seed],
                        ])
                        try:
                            fit_res, J2_try, sig_try, r2_curve = _r2s2_fit(
                                seed_state, R, cp, f2, Nmax=12000,
                                h=0.0005)
                        except Exception:
                            continue
                        if J2_try is None or len(r2_curve) < 5:
                            continue
                        sig_chk, rh_chk = compute_shock_speed(J2_try, R,
                                                              cp, f2)
                        try:
                            _, lam2_chk, _, _, _ = compute_eigenvalues(
                                J2_try[0], J2_try[2], cp)
                        except Exception:
                            lam2_chk = np.nan
                        if (np.isfinite(sig_try) and np.isfinite(sig_chk) and
                                fit_res < 2e-2 and rh_chk < 2e-2 and
                                abs(sig_chk - lam2_chk) < 8e-2 and
                                sig_try > xi_n[k_seed]):
                            seeded = (k_seed, seed_state, fit_res, J2_try,
                                      float(sig_chk), r2_curve, float(lam2_chk))
                            break

                    if seeded is not None:
                        (k_seed, seed_state, fit_res, J2, sig_J2R,
                         r2_curve, lam2_J2) = seeded
                        dJ = ((r2_curve[:, 0] - J2[0])**2 +
                              (r2_curve[:, 2] - J2[2])**2)
                        iJ = int(np.argmin(dJ))
                        rar_seg = r2_curve[:iJ + 1].copy()
                        if len(rar_seg) > 0:
                            # The Welge residual is tiny but not exactly zero;
                            # draw the terminal S2 shock as a vertical jump at
                            # the RH speed sigma(J2,R).
                            rar_seg[-1, 3] = sig_J2R

                        post_Sw = Sw_n[iB:k_seed + 1]
                        post_Sg = Sg_n[iB:k_seed + 1]
                        post_xi = xi_n[iB:k_seed + 1].copy()
                        if len(post_xi) > 0:
                            post_lo = max(float(sig_AB_eff) + 1e-3,
                                          float(post_xi[0]))
                            post_hi = float(rar_seg[0, 3])
                            if len(post_xi) >= 2 and post_hi > post_lo:
                                raw = post_xi.copy()
                                den = max(float(raw[-1] - raw[0]), 1e-12)
                                post_xi = post_lo + (raw - raw[0]) / den * \
                                    (post_hi - post_lo)
                            else:
                                post_xi = np.maximum.accumulate(post_xi)
                                post_xi[0] = max(post_xi[0], post_lo)
                        fast_Sw = np.concatenate([
                            [M_trans[0]], post_Sw, rar_seg[1:, 0], [R[0]]
                        ])
                        fast_Sg = np.concatenate([
                            [M_trans[2]], post_Sg, rar_seg[1:, 2], [R[2]]
                        ])
                        fast_xi = np.concatenate([
                            [float(sig_AB_eff) + 1e-3], post_xi,
                            rar_seg[1:, 3], [sig_J2R]
                        ])
                        fast_xi = np.maximum.accumulate(fast_xi)
                        fast_path = np.column_stack([
                            fast_Sw, 1.0 - fast_Sw - fast_Sg,
                            fast_Sg, fast_xi
                        ])
                        fast_type = 'fast-rarefaction-shock'
                        print(f"    [num-transition-split] seeded R2/S2 "
                              f"closure from idx {k_seed}: "
                              f"seed=({seed_state[0]:.4f},"
                              f"{seed_state[1]:.4f},{seed_state[2]:.4f}) "
                              f"J2=({J2[0]:.4f},{J2[1]:.4f},"
                              f"{J2[2]:.4f}) sigma={sig_J2R:.4f} "
                              f"fit={fit_res:.2e}")
                    else:
                        # Last-resort closure: reuse the terminal R2/S2
                        # Welge point from the rejected R1S1R2S2 candidate.
                        # Its slow side is invalid here, but its final
                        # fast-shock point is RH/characteristic matched.
                        cand = getattr(find_intermediate_state,
                                       '_rsrs_candidate', None)
                        if cand is not None and cand.get('u_star_2') is not None:
                            J2 = cand['u_star_2'].copy()
                            sig_J2R = float(cand['sigma2'])
                            try:
                                _, lam2_J2, _, _, _ = compute_eigenvalues(
                                    J2[0], J2[2], cp)
                            except Exception:
                                lam2_J2 = sig_J2R
                            post_Sw = Sw_n[iB:iR_start + 1]
                            post_Sg = Sg_n[iB:iR_start + 1]
                            post_xi = xi_n[iB:iR_start + 1].copy()
                            if len(post_xi) > 0 and \
                                    post_xi[0] < sig_AB_eff + 1e-9:
                                shift = sig_AB_eff + 1e-3 - post_xi[0]
                                post_xi = post_xi + max(shift, 0.0)
                            fast_Sw = np.concatenate([
                                [M_trans[0]], post_Sw, [J2[0], R[0]]
                            ])
                            fast_Sg = np.concatenate([
                                [M_trans[2]], post_Sg, [J2[2], R[2]]
                            ])
                            fast_xi = np.concatenate([
                                [float(sig_AB_eff) + 1e-3], post_xi,
                                [sig_J2R, sig_J2R]
                            ...
                            fast_type = 'fast-rarefaction-shock'
                            print(f"    [num-transition-split] using saved "
                                  f"R1S1R2S2 terminal J2: sigma={sig_J2R:.4f}")
                        else:
                            # Fallback of last resort: keep the short
                            # numerical post-B profile, but mark the resulting
                            # speed as diagnostic rather than physical.
                            post_Sw = Sw_n[iB:iR_start + 1]
                            post_Sg = Sg_n[iB:iR_start + 1]
                            post_xi = xi_n[iB:iR_start + 1].copy()
                            if len(post_xi) > 0 and \
                                ...
                                shift = sig_AB_eff + 1e-3 - post_xi[0]
                                post_xi = post_xi + max(shift, 0.0)
                            sig_BR, _ = compute_shock_speed(M_trans, R,
                                                            cp, f2)
                            sig_BR = float(sig_BR)
                            last_post_xi = float(post_xi[-1]) \
                                if len(post_xi) else sig_AB_eff
                            if not np.isfinite(sig_BR) or sig_BR <= last_post_xi:
                                sig_BR = last_post_xi + max(
                                    0.5, 0.1 * last_post_xi)
                            fast_Sw = np.concatenate([
                                [M_trans[0]], post_Sw, [R[0]]
                            ])
                            fast_Sg = np.concatenate([
                                [M_trans[2]], post_Sg, [R[2]]
                            ])
                            fast_xi = np.concatenate([
                                [sig_AB_eff + 1e-3], post_xi, [sig_BR]
                            ])
                            fast_xi = np.maximum.accumulate(fast_xi)
                            fast_path = np.column_stack([
                                fast_Sw, 1.0 - fast_Sw - fast_Sg,
                            ...
                            fast_type = 'fast-rarefaction-shock'
                            J2 = fast_path[-2, :3].copy()
                            sig_J2R = float(fast_xi[-1])
                            lam2_J2 = np.nan

                # Prefer the structure indicated by the resolved profile:
                #   L -> P       boundary slow rarefaction on So=0
                #   P -> Q       entry shock into the three-phase region
                #   Q -> A       fast-family branch
                #   A -> B       transitional shock across the elliptic patch
                #   B -> J2 -> R fast rarefaction + terminal shock
                #
                # The previous implementation put L->A into the slow channel,
                # but the numerical profile has xi ~= lambda_2 almost
                # immediately after the oil phase appears.  Keep A/B as an
                # internal fast-family transition, not as the slow/fast split.
                profile = np.column_stack([
                    Sw_n, 1.0 - Sw_n - Sg_n, Sg_n, xi_n
                ])
                entry = _find_boundary_entry_shock_on_profile(
                    L, profile, cp, f2)
                if entry is not None and int(entry['idx_Q']) < iA:
                    iQ = int(entry['idx_Q'])
                    Q = entry['Q'].copy()
                    q_start_xi = max(float(entry['lam_Q']),
                                     float(entry['sig_PQ']) + 1e-4)
                    eps_trans = max(1e-7, abs(sig_AB_eff) * 1e-6)
                    q_row = np.array([[Q[0], Q[1], Q[2], q_start_xi]])

                    qa_seg = profile[iQ:iA + 1].copy()
                    qa_mask = ((qa_seg[:, 3] > q_start_xi + 1e-8) &
                               (qa_seg[:, 3] < sig_AB_eff - eps_trans))
                    qa_seg = qa_seg[qa_mask]

                    # End the pre-transition fast branch at A just before the
                    # A->B transitional jump.
                    if len(qa_seg) > 0:
                        a_xi = min(sig_AB_eff - eps_trans,
                                   max(float(qa_seg[-1, 3]) + eps_trans,
                                       q_start_xi + eps_trans))
                    else:
                        a_xi = min(sig_AB_eff - eps_trans,
                                   q_start_xi + eps_trans)
                    a_row = np.array([[A[0], A[1], A[2], a_xi]])
                    trans_rows = np.array([
                        [A[0], A[1], A[2], sig_AB_eff],
                        [M_trans[0], M_trans[1], M_trans[2], sig_AB_eff],
                    ])

                    # The B->... closure already starts just after sigma_AB.
                    b_tail = fast_path.copy()
                    b_tail = b_tail[b_tail[:, 3] > sig_AB_eff + 1e-9]

                    rar_QR = np.vstack([
                        q_row, qa_seg, a_row, trans_rows, b_tail
                    ])
                    result = _finalize_boundary_entry_solution(
                        L, R, M_trans, entry, rar_QR, cp, f2)
                    result['M_legacy'] = M_legacy
                    result['A_transition'] = A
                    result['B_transition'] = M_trans
                    result['J2_fast'] = J2
                    result['transition_shock_info'] = {
                        'A': A, 'B': M_trans,
                        'sigma_AB': float(sig_AB_eff),
                        'rh_AB': float(rh_AB), 'iA': int(iA), 'iB': int(iB),
                        'J2': J2, 'sigma_J2R': float(sig_J2R),
                        'lambda2_J2': (float(lam2_J2)
                                       if np.isfinite(lam2_J2) else np.nan),
                        'M_legacy': M_legacy,
                    }
                    print("    [entry-fast-transition] using "
                          "boundary-rarefaction + entry shock + fast "
                          "transition + terminal shock")
                    return result

                print(f"    [num-transition-split] slow: L->A rarefaction + "
                      f"A->B transitional shock; sigma(A,B)={sig_AB_eff:.4f}")
                print(f"    [num-transition-split] A=({A[0]:.4f},{A[1]:.4f},"
                      f"{A[2]:.4f})  B/M=({M_trans[0]:.4f},{M_trans[1]:.4f},"
                      f"{M_trans[2]:.4f})  old M=({M_legacy[0]:.4f},"
                      f"{M_legacy[1]:.4f},{M_legacy[2]:.4f})")
                result = _finalize_solution(L, R, M_trans, slow_path, slow_type,
                                            slow_xi, slow_Sw, slow_Sg,
                                            fast_path, fast_type,
                                            fast_xi, fast_Sw, fast_Sg, cp, f2)
                result['M_legacy'] = M_legacy
                ...
                result['J2_fast'] = J2
                result['transition_shock_info'] = {
                    'A': A, 'B': M_trans, 'sigma_AB': float(sig_AB_eff),
                    'rh_AB': float(rh_AB), 'iA': int(iA), 'iB': int(iB),
                    'J2': J2, 'sigma_J2R': float(sig_J2R),
                    'lambda2_J2': float(lam2_J2) if np.isfinite(lam2_J2) else np.nan,
                    'M_legacy': M_legacy,
                }
                return result

            # No elliptic crossing in the numerical profile -- old structure
            # still applies; fall through to legacy boundary / num-slice /
            # double-shock fallbacks below.
            print("    [num-fast-trans] no elliptic crossing in numerical; "
                  "falling through to legacy strategies")

            # ---- Legacy fallback 1: L on So=SORG edge with boundary R1
            L_on_SORG_edge = (abs(L[1] - SORG) <= 0.025)
            if L_on_SORG_edge:
                J1_b, sig_J1M_b, lam_edge_J1, info_b = \
                    _find_J1_on_boundary_to_M(L, M, cp, f2)
                if J1_b is not None:
                    fast_pack = _make_fast_composite_MR(M, R, cp, f2)
                    if fast_pack[0] is not None:
                        (fast_path_b, fast_xi_b, fast_Sw_b, fast_Sg_b,
                         J2_b, sig_J2R_b, lam2_J2_b) = fast_pack
                        try:
                            _, lam2_M_val, _, _, _ = compute_eigenvalues(
                                M[0], M[2], cp)
                        except Exception:
                            lam2_M_val = float(fast_xi_b[0])
                        lam2_M_val = float(lam2_M_val)

                        # ---- Slow channel: BL rarefaction L->J1 then
                        # transitional shock J1->M ----
                        Sw_grid = np.linspace(L[0], J1_b[0], 80)
                        xi_rar_raw = np.array(
                            [_boundary_wave_speed(s, cp, f2)
                             for s in Sw_grid])
                        # Trim trailing samples that are NOT strictly
                        # increasing in xi (np.unique would otherwise
                        # collapse them and the renderer would clip the
                        # rarefaction visually short).  Keep only the
                        # initial run with strictly increasing xi.
                        keep = [0]
                        for k in range(1, len(xi_rar_raw)):
                            if xi_rar_raw[k] > xi_rar_raw[keep[-1]] + 1e-9:
                                keep.append(k)
                            else:
                                break
                        keep = np.array(keep, dtype=int)
                        Sw_rar = Sw_grid[keep]
                        Sg_rar = 1.0 - Sw_grid[keep] - SORG
                        xi_rar = xi_rar_raw[keep]

                        # Append the transitional shock J1->M at the
                        # FAST leading characteristic xi=lam_2(M).  Slow
                        # trailing rar char and fast leading rar char then
                        # coincide there, giving a self-similar solution
                        # with monotone xi.  The actual chord speed sig_J1M
                        # may differ (transitional shocks are not Lax-1).
                        xi_rar_end = float(xi_rar[-1])
                        sig_J1M_eff = max(lam2_M_val, xi_rar_end + 1e-3)
                        Sw_slow = np.concatenate([Sw_rar, [M[0]]])
                        Sg_slow = np.concatenate([Sg_rar, [M[2]]])
                        xi_slow = np.concatenate([xi_rar, [sig_J1M_eff]])
                        slow_path = np.column_stack([
                            Sw_slow, 1.0 - Sw_slow - Sg_slow, Sg_slow,
                            xi_slow
                        ])
                        slow_xi, slow_Sw, slow_Sg = xi_slow, Sw_slow, Sg_slow
                        slow_type = 'compound'
                        print(f"    [bndry-trans] slow rar samples kept: "
                              f"{len(xi_rar)}  (Sw {Sw_rar[0]:.3f}->"
                              f"{Sw_rar[-1]:.3f}, xi {xi_rar[0]:.4f}->"
                              f"{xi_rar[-1]:.4f}); slow shock rendered at "
                              f"xi={sig_J1M_eff:.4f} (lam_2(M)="
                              f"{lam2_M_val:.4f}, chord sig={sig_J1M_b:.4f})")

                        # ---- Fast channel: from _make_fast_composite_MR
                        fast_path = fast_path_b
                        fast_xi, fast_Sw, fast_Sg = \
                            fast_xi_b, fast_Sw_b, fast_Sg_b
                        fast_type = 'composite'

                        # Wave-speed ordering check
                        if float(slow_xi[-1]) >= float(fast_xi[0]):
                            print(f"    [bndry-trans] WARNING wave speeds "
                                  f"overlap: slow_max={slow_xi[-1]:.4f} >= "
                                  f"fast_min={fast_xi[0]:.4f}")

                        print(f"    [bndry-trans] slow: BL rar L->J1 "
                              f"(Sw {L[0]:.3f}->{J1_b[0]:.3f}) + trans "
                              f"shock J1->M at sigma={sig_J1M_b:.4f}")
                        print(f"    [bndry-trans] fast: rar M->J2 + shock "
                              f"J2->R at sigma={sig_J2R_b:.4f}, "
                              f"J2=({J2_b[0]:.4f},{J2_b[1]:.4f},"
                              f"{J2_b[2]:.4f})")
                        return _finalize_solution(L, R, M,
                                                   slow_path, slow_type,
                                                   slow_xi, slow_Sw, slow_Sg,
                                                   fast_path, fast_type,
                                                   fast_xi, fast_Sw, fast_Sg,
                        ...
                        print("    [bndry-trans] J1 found but fast composite "
                              "failed; falling through")
                else:
                    print("    [bndry-trans] no J1 on boundary; falling through")

            # ---- First try: analytical transitional shock J->K crossing
            # the elliptic region.  This replaces the [zero-strength]+
            # [fast-composite] numerical-profile fallback whenever the L
            # and M fast integral curves both terminate at the elliptic
            # boundary and we can find a doubly-characteristic shock J->K
            # with sigma=lam2(J)=lam2(K) bridging them.  See
            # diag_fast_composite_admiss.py for the motivation.
            J_t, K_t, rar_LJ, rar_KM, sig_JK, info_t = \
                _find_trans_shock_across_elliptic(L, M, R, cp, f2)
            if J_t is not None:
                print("    [trans-elliptic] using rar(L->J) + shock(J->K) + "
                      "rar(K->M) + shock(M->R) construction")
                # ---- Slow channel: zero-strength jump at L
                lam2_L_val = float(rar_LJ[0, 3])
                slow_xi = np.array([lam2_L_val, lam2_L_val])
                slow_Sw = np.array([L[0], L[0]])
                slow_Sg = np.array([L[2], L[2]])
                slow_path = np.array([
                    [L[0], L[1], L[2], lam2_L_val],
                    [L[0], L[1], L[2], lam2_L_val],
                ])
                slow_type = 'zero-strength'

                # ---- Fast channel: rar(L->J) then shock J->K (at sig_JK)
                # then rar(K->M) then shock M->R (at sig_MR)
                sig_MR_val, _ = compute_shock_speed(M, R, cp, f2)
                sig_MR_val = float(sig_MR_val)

                # rar(L->J): xi = lam_2 ascending up to lam_2(J)
                xi_LJ = rar_LJ[:, 3].copy()
                Sw_LJ = rar_LJ[:, 0].copy()
                Sg_LJ = rar_LJ[:, 2].copy()
                # Force monotone non-decreasing in xi for plotting
                xi_LJ = np.maximum.accumulate(xi_LJ)

                # rar(K->M): xi from lam_2(K) ascending to lam_2(M)
                xi_KM = rar_KM[:, 3].copy()
                Sw_KM = rar_KM[:, 0].copy()
                Sg_KM = rar_KM[:, 2].copy()
                xi_KM = np.maximum.accumulate(xi_KM)

                # The transitional shock J->K is doubly characteristic, so
                # sig_JK ~ lam_2(J) ~ lam_2(K).  Render it as a single
                # discontinuity with two samples at xi = sig_JK.
                # Glue the segments together.  Use [last point of rar_LJ
                # at xi=sig_JK; first point of rar_KM at xi=sig_JK] as the
                # explicit shock samples.
                xi_J_eff = max(xi_LJ[-1], sig_JK)
                xi_LJ[-1] = xi_J_eff  # ensure shock-end matches sig_JK
                xi_KM[0] = max(xi_KM[0], sig_JK)
                # If KM's first xi is below sig_JK (shouldn't happen by
                # construction), bump it
                xi_KM = np.maximum.accumulate(xi_KM)

                # Concatenate: rar_LJ ... shock(J->K) ... rar_KM ... shock(M->R) ... R
                fast_Sw_arr = np.concatenate([Sw_LJ, Sw_KM, [R[0]]])
                fast_Sg_arr = np.concatenate([Sg_LJ, Sg_KM, [R[2]]])
                fast_xi_arr = np.concatenate([xi_LJ, xi_KM, [sig_MR_val]])

                # Strict monotonicity in xi for plotting (np.interp etc.)
                # Use cumulative max so duplicates collapse but xi never
                # decreases.
                fast_xi_arr = np.maximum.accumulate(fast_xi_arr)
                xi_u, idx_u = np.unique(fast_xi_arr, return_index=True)
                Sw_u = fast_Sw_arr[idx_u]
                Sg_u = fast_Sg_arr[idx_u]

                fast_xi, fast_Sw, fast_Sg = xi_u, Sw_u, Sg_u
                fast_path = np.column_stack([
                    Sw_u, 1.0 - Sw_u - Sg_u, Sg_u, xi_u
                ])
                fast_type = 'composite (rar+trans-shock+rar+shock)'

                print(f"    [trans-elliptic] sigma(J,K)={sig_JK:.4f}  "
                      f"sigma(M,R)={sig_MR_val:.4f}")
                print(f"    [trans-elliptic] J=({J_t[0]:.4f},"
                      f"{J_t[1]:.4f},{J_t[2]:.4f}) "
                      f"K=({K_t[0]:.4f},{K_t[1]:.4f},{K_t[2]:.4f})")
            ...
            # ---- Second try: numerical-sliced R1S1 + R2S2 (compound slow
            # + composite fast).  The numerical viscous profile traces
            #     L --(slow rar)--> J1 --(slow shock at sigma1)--> M
            #       --(fast rar)--> J2 --(fast shock at sigma2)--> R
            # which is the Juanes-Patzek B.3 wave structure.  When BOTH
            # sub-segments show meaningful arclength (i.e., the L->M and
            # M->R numerical paths are genuinely curved, not collinear
            # with the chord), slice the numerical profile at M and feed
            # the two halves into the renderer as compound/composite so
            # the rar→shock kinks show up correctly.  This faithfully
            # reproduces the numerical even when the strict-analytical
            # search for J1, J2 (Strategies 1d, R1S1R2S2) didn't converge.
            from numerical_solver import run_numerical
            num_chk = run_numerical(L, R, cp, Nx=3000, t_final=0.4,
                                    quiet=True)
            xi_chk = num_chk['xi']; Sw_chk = num_chk['Sw']; Sg_chk = num_chk['Sg']

            # Trim L and R plateaus
            d2L_chk = (Sw_chk - L[0])**2 + (Sg_chk - L[2])**2
            off_L_chk = d2L_chk > 0.005**2
            iLp = int(np.argmax(off_L_chk)) if off_L_chk.any() else 1
            iLp = max(iLp, 1)
            d2R_chk = (Sw_chk - R[0])**2 + (Sg_chk - R[2])**2
            close_R_chk = d2R_chk < 0.01**2
            iRp = (int(np.argmax(close_R_chk))
                   if close_R_chk.any() else len(xi_chk) - 1)
            if iRp <= iLp + 10:
                iRp = len(xi_chk) - 1

            # Locate M in the numerical profile
            d2M_chk = (Sw_chk[iLp:iRp + 1] - M[0])**2 \
                      + (Sg_chk[iLp:iRp + 1] - M[2])**2
            iM_local = int(np.argmin(d2M_chk))
            iM = iLp + iM_local
            dist_M_to_num = float(np.sqrt(d2M_chk[iM_local]))

            # Curvature gauges: compare arclength of each half to its chord
            def _curve_offset(Sw_seg, Sg_seg, A, B):
                if len(Sw_seg) < 3:
                    return 0.0
                ax_, ay_ = A[0], A[2]
                bx_, by_ = B[0], B[2]
                ...
                if vlen2 < 1e-12:
                    return 0.0
                # Perpendicular distance of each interior point from chord A-B
                offs = []
                for i in range(1, len(Sw_seg) - 1):
                    px, py = Sw_seg[i] - ax_, Sg_seg[i] - ay_
                    cross = abs(px * vy - py * vx) / np.sqrt(vlen2)
                    offs.append(cross)
                return float(np.max(offs)) if offs else 0.0

            slow_off = _curve_offset(Sw_chk[iLp:iM + 1],
                                     Sg_chk[iLp:iM + 1], L, M)
            fast_off = _curve_offset(Sw_chk[iM:iRp + 1],
                                     Sg_chk[iM:iRp + 1], M, R)
            # Threshold: if either half's max chord-offset > 0.03 in (Sw,Sg),
            # treat it as "genuinely curved" (rarefaction-rich)
            print(f"    [num-slice] iM={iM} (dist M<->profile={dist_M_to_num:.4f})  "
                  f"slow off-chord max={slow_off:.4f}  "
                  f"fast off-chord max={fast_off:.4f}")
            curved_slow = slow_off > 0.03
            curved_fast = fast_off > 0.03

            if (curved_slow or curved_fast) and dist_M_to_num < 0.05:
                print("    [num-slice] using compound slow + composite fast "
                      "(numerical L->M and M->R sliced at M)")

                sig_LM_eff, _ = compute_shock_speed(L, M, cp, f2)
                sig_MR_eff, _ = compute_shock_speed(M, R, cp, f2)
                sig_LM_eff = float(sig_LM_eff)
                sig_MR_eff = float(sig_MR_eff)

                # ---- Slow channel: L + numerical [iLp:iM] + (M, sigma_LM)
                # Endpoint shock M is rendered by appending [M, sigma_LM]
                # so the renderer's xi-jump detector flags it as a shock.
                slow_Sw_seg = np.concatenate(
                    [[L[0]], Sw_chk[iLp:iM + 1], [M[0]]])
                slow_Sg_seg = np.concatenate(
                    [[L[2]], Sg_chk[iLp:iM + 1], [M[2]]])
                slow_xi_seg = np.concatenate(
                    [[xi_chk[iLp]], xi_chk[iLp:iM + 1], [sig_LM_eff]])
                # Enforce non-decreasing xi
                slow_xi_seg = np.maximum.accumulate(slow_xi_seg)
                # Drop duplicates
                slow_xi_u, idx_su = np.unique(slow_xi_seg, return_index=True)
                slow_Sw_u = slow_Sw_seg[idx_su]
                slow_Sg_u = slow_Sg_seg[idx_su]
                slow_path = np.column_stack([
                    slow_Sw_u, 1.0 - slow_Sw_u - slow_Sg_u,
                    slow_Sg_u, slow_xi_u
                ])
                slow_xi, slow_Sw, slow_Sg = slow_xi_u, slow_Sw_u, slow_Sg_u
                slow_type = 'compound'

                # ---- Fast channel: composite if numerical M->R is curved,
                # otherwise a pure Lax-2 shock at sigma(M,R).
                if curved_fast:
                    fast_Sw_seg = np.concatenate(
                        [[M[0]], Sw_chk[iM:iRp + 1], [R[0]]])
                    fast_Sg_seg = np.concatenate(
                        [[M[2]], Sg_chk[iM:iRp + 1], [R[2]]])
                    fast_xi_seg = np.concatenate(
                        [[xi_chk[iM]], xi_chk[iM:iRp + 1], [sig_MR_eff]])
                    fast_xi_seg = np.maximum.accumulate(fast_xi_seg)
                    fast_xi_u, idx_fu = np.unique(fast_xi_seg,
                                                   return_index=True)
                    fast_Sw_u = fast_Sw_seg[idx_fu]
                    fast_Sg_u = fast_Sg_seg[idx_fu]
                    fast_path = np.column_stack([
                        fast_Sw_u, 1.0 - fast_Sw_u - fast_Sg_u,
                        fast_Sg_u, fast_xi_u
                    ])
                    fast_xi, fast_Sw, fast_Sg = fast_xi_u, fast_Sw_u, fast_Sg_u
                    fast_type = 'composite'
                else:
                    fast_path, fast_type, fast_xi, fast_Sw, fast_Sg = \
                        _make_shock(M, R, cp, f2)

                print(f"    [num-slice] slow: {len(slow_xi)} samples,  "
                      f"sigma(L,M)={sig_LM_eff:.4f}  "
                      f"(transitional - crosses elliptic, rh={rh_LM_chk:.2e})"
                      if False else
                      f"    [num-slice] slow: {len(slow_xi)} samples,  "
                      f"sigma(L,M)={sig_LM_eff:.4f}")
                print(f"    [num-slice] fast: {len(fast_xi)} samples,  "
                      f"sigma(M,R)={sig_MR_eff:.4f}")
                return _finalize_solution(L, R, M, slow_path, slow_type,
                                           slow_xi, slow_Sw, slow_Sg,
                                           fast_path, fast_type,
                                           fast_xi, fast_Sw, fast_Sg, cp, f2)

            # ---- Third try: clean S1S2_trans double-shock when L,M are
            # NOT on a common fast integral curve and both numerical
            # halves are essentially straight chords.
            sig_LM_chk, rh_LM_chk = compute_shock_speed(L, M, cp, f2)
            sig_MR_chk, rh_MR_chk = compute_shock_speed(M, R, cp, f2)
            if (np.isfinite(sig_LM_chk) and np.isfinite(sig_MR_chk)
                    and rh_LM_chk < 0.05 and rh_MR_chk < 0.05
                    and sig_LM_chk < sig_MR_chk):
                print(f"    [trans-elliptic] no rar+trans+rar structure; "
                      f"both numerical halves nearly straight; using clean "
                      f"double-shock L->M (trans, rh={rh_LM_chk:.2e}) + "
                      f"M->R (rh={rh_MR_chk:.2e})")
                slow_path, slow_type, slow_xi, slow_Sw, slow_Sg = \
                    _make_shock(L, M, cp, f2)
                fast_path, fast_type, fast_xi, fast_Sw, fast_Sg = \
                    _make_shock(M, R, cp, f2)
                if slow_xi[0] >= fast_xi[0]:
                    mid = 0.5 * (float(slow_xi[0]) + float(fast_xi[0]))
                    slow_xi = np.array([mid, mid])
                    fast_xi = np.array([mid, mid])
                print(f"    [double-shock] sigma(L,M)={sig_LM_chk:.4f} "
                      f"(transitional, crosses elliptic)  "
                      f"sigma(M,R)={sig_MR_chk:.4f}")
                return _finalize_solution(L, R, M, slow_path, slow_type,
                                           slow_xi, slow_Sw, slow_Sg,
                                           fast_path, fast_type,
                                           fast_xi, fast_Sw, fast_Sg, cp, f2)

            # ---- Final fallback: analytical fast-composite with explicit
            # interior Liu shock at the V_2 inflection (slow family carries
            # zero strength).  Falls back to numerical-profile rendering
            # only when the analytical helper rejects the geometry (curve
            # from M doesn't reach L within tolerance, lam_2 monotone,
            # RH residual too large, ...).
            #     L =[zero-strength jump]= L_eq
            #         --R_2--> u_a (V_2, lambda_2 local max)
            #         --S_2 interior at sigma=lambda_2(u_a)--> u_b
            #         --R_2--> M
            #         --S_2 terminal (Lax-2)--> R
            print("    [fast_composite] trans-elliptic and double-shock "
                  "both rejected; building analytical fast-composite "
                  "(R_2 + interior S_2 + R_2 + terminal S_2)")
            info_fc = _build_analytical_fast_composite(L, M, cp, f2)
            if info_fc is None:
                print("    [fast_composite] analytical construction failed; "
                      "falling back to numerical profile")
                return _fast_composite_from_numerical(L, R, M, cp, f2)

            sig_term, _ = compute_shock_speed(M, R, cp, f2)
            sig_term = float(sig_term)
            rar1 = info_fc['rar1']
            rar2 = info_fc['rar2']
            sigma_int = float(info_fc['sigma_int'])
            L_eq = info_fc['L_eq']
            u_a = info_fc['u_a']
            u_b = info_fc['u_b']
            lam2_M = float(info_fc['lam2_M'])
            if not np.isfinite(sig_term) or sig_term <= lam2_M:
                sig_term = lam2_M + max(0.5, 0.1 * abs(lam2_M))

            slow_xi_val = float(rar1[0, 3])
            slow_xi = np.array([slow_xi_val, slow_xi_val])
            slow_Sw = np.array([L[0], L_eq[0]])
            slow_Sg = np.array([L[2], L_eq[2]])
            slow_path = np.array([
            ...
            fast_path = np.vstack([
                rar1,
                np.array([[u_a[0], u_a[1], u_a[2], sigma_int],
                          [u_b[0], u_b[1], u_b[2], sigma_int]]),
                rar2,
                np.array([[M[0], M[1], M[2], sig_term],
                          [R[0], R[1], R[2], sig_term]]),
            ])
            fast_xi = fast_path[:, 3].copy()
            for i in range(1, len(fast_xi)):
                if fast_xi[i] < fast_xi[i - 1]:
                    fast_xi[i] = fast_xi[i - 1]
            fast_Sw = fast_path[:, 0].copy()
            fast_Sg = fast_path[:, 2].copy()
            fast_type = 'fast-composite'

            print(f"    [fast_composite] L_eq=({L_eq[0]:.4f},{L_eq[1]:.4f},"
                  f"{L_eq[2]:.4f})  u_a=({u_a[0]:.4f},{u_a[1]:.4f},"
                  f"{u_a[2]:.4f})  u_b=({u_b[0]:.4f},{u_b[1]:.4f},"
                  f"{u_b[2]:.4f})")
            print(f"    [fast_composite] sigma_interior={sigma_int:.4f}  "
                  f"sigma_terminal(M,R)={sig_term:.4f}  "
                  f"rar1={len(rar1)} pts  rar2={len(rar2)} pts")
    ...
    # Trace the fast integral curve in BOTH directions because the sign
    # of the eigenvector is arbitrary -- one direction heads toward R,
    ...
    fast_m = integrate_rarefaction(M[0], M[2], 2, -1, cp, f2,
                                   Nmax=5000, h=0.0008)
    if len(fast_p) > 0 and len(fast_m) > 0:
        # Pick the branch whose trajectory passes closest to R.
        # The old heuristic (net lambda_2 increase) fails when the
        # rarefaction crosses the inflection locus: net increase is
        # negative even though the curve sweeps through high lam2
        # values and passes near R.
        def _min_d2(arr):
            return float(np.min((arr[:, 0] - R[0])**2 +
                                (arr[:, 2] - R[2])**2))
        fast_rar = fast_p if _min_d2(fast_p) <= _min_d2(fast_m) else fast_m
    elif len(fast_p) > 0:
        fast_rar = fast_p
    elif len(fast_m) > 0:
        fast_rar = fast_m
...
def _finalize_solution(L, R, M, slow_path, slow_type, slow_xi, slow_Sw, slow_Sg,
    ...
    # -- Diagnostic: wave speed along L -> M -> R --------------------------
    def _dump(path_arr):
        if path_arr is None or len(path_arr) == 0:
            return
        n = len(path_arr)
        if n > 12:
            idxs = sorted(set([0]
                              + list(np.linspace(1, n - 2, 8).astype(int))
                              + [n - 2, n - 1]))
        else:
            idxs = list(range(n))
        print("     i    Sw     So     Sg       xi       lam1      lam2")
    ...
    _lam1_L, _lam2_L, _, _, _ = compute_eigenvalues(L[0], L[2], cp)
    _lam1_M, _lam2_M, _, _, _ = compute_eigenvalues(M[0], M[2], cp)
    _lam1_R, _lam2_R, _, _, _ = compute_eigenvalues(R[0], R[2], cp)
    _sig_LM, _ = compute_shock_speed(L, M, cp, f2)
    _sig_MR, _ = compute_shock_speed(M, R, cp, f2)

    print("  --- Slow wave (L -> M) speed table ---")
    print(f"    type={slow_type}  xi range [{float(np.min(slow_xi)):.4f}, "
          f"{float(np.max(slow_xi)):.4f}]")
    _dump(slow_path)
    print(f"    lam1(L)={_lam1_L:.4f}  lam1(M)={_lam1_M:.4f}  "
          f"sigma(L,M)={_sig_LM:.4f}")

    print("  --- Fast wave (M -> R) speed table ---")
    print(f"    type={fast_type}  xi range [{float(np.min(fast_xi)):.4f}, "
          f"{float(np.max(fast_xi)):.4f}]")
    _dump(fast_path)
    print(f"    lam2(M)={_lam2_M:.4f}  lam2(R)={_lam2_R:.4f}  "
          f"sigma(M,R)={_sig_MR:.4f}")
    # --------------------------------------------------------------------
...
def _build_wave_on_curve(curve, sa, sb, fam, cp, f2):
    ...
    # Diagnostic: show where the Welge tangent lives on the integral curve
    _ds_rar = float(np.hypot(Sw_u[-1] - Sw_u[0], Sg_u[-1] - Sg_u[0]))
    _ds_sh = float(np.hypot(sb[0] - Sw_u[-1], sb[2] - Sg_u[-1]))
    print(f"      [_build_wave_on_curve] fam={fam} composite: "
          f"rar {len(Sw_u)} pts |dS|={_ds_rar:.4f}, "
          f"shock |dS|={_ds_sh:.4f} from "
          f"u*=({Sw_u[-1]:.4f},{Sg_u[-1]:.4f}) to "
          f"({sb[0]:.4f},{sb[2]:.4f}) at sigma={sig_sh:.4f}")
    # Build a path array that reflects the COMPOSITE geometry in (Sw,Sg):
    # rarefaction points along the integral curve, followed by the shock
    # jump from shock_start to shock_end (=sb).  Previously this returned
    # the full integral-curve slice from sa to sb, which hid the shock
    # because all points stayed on the integral curve.  The plotter uses
    # the |dS| heuristic (>10x median step) to split solid/dashed; the
    # jump from shock_start to sb triggers that when it is non-trivial.
    comp_rows = np.column_stack([Sw_u, 1.0 - Sw_u - Sg_u, Sg_u, xi_u])
    shock_row = np.array([[sb[0], sb[1], sb[2], sig_sh]])
    path_comp = np.vstack([comp_rows, shock_row])
    return path_comp, 'composite', xi_comb, Sw_comb, Sg_comb
...
def compute_inflection_loci(cp, ng=80):
    ...
    """
    h = 2e-4              # finite-difference stencil size
    # Margin from physical boundaries = exactly one stencil width.  Previously
    # an arbitrary eps=5e-4 was used, which hid a band of width 5e-4 along
    # the So=0 edge from the V_i computation entirely.  Within that band the
    # composite slow wave near L has its post-rarefaction state on V_1, so
    # masking it out made V_1 appear absent near L.
    Sw_v = np.linspace(SWC + h, 1.0 - h, ng)
    Sg_v = np.linspace(h,        1.0 - SWC - h, ng)
    ...
    for i in range(ng):
        for j in range(ng):
            Sw, Sg = Sw_v[i], Sg_v[j]
            So = 1.0 - Sw - Sg
            # Only require the centre point to lie inside the simplex; the
            # FD stencil is allowed to fall back to one-sided when a
            # symmetric step would leave the physical region.
            if So < -1e-12 or Sw < SWC - 1e-12 or Sg < -1e-12:
                continue
            try:
                ...
                # +Sw step: So -> So - h   (need So >= h)
                # -Sw step: Sw -> Sw - h   (need Sw - h >= SWC)
                ok_pw = So >= h - 1e-12
                ok_mw = Sw - h >= SWC - 1e-12
                if not (ok_pw or ok_mw):
                    continue
                if ok_pw and ok_mw:
                    l1_pw, l2_pw = _eig(Sw + h, Sg)
                    l1_mw, l2_mw = _eig(Sw - h, Sg)
                    den_w = 2.0 * h
                elif ok_pw:
                    l1_pw, l2_pw = _eig(Sw + h, Sg)
                    l1_mw, l2_mw = l1, l2
                    ...
                    l1_mw, l2_mw = _eig(Sw - h, Sg)
                    den_w = h
                dl1_dSw = (l1_pw - l1_mw) / den_w
                dl2_dSw = (l2_pw - l2_mw) / den_w

                # +Sg step: So -> So - h   (need So >= h)
                # -Sg step: Sg -> Sg - h   (need Sg - h >= 0)
                ok_pg = So >= h - 1e-12
                ok_mg = Sg - h >= -1e-12
                if not (ok_pg or ok_mg):
                    continue
                if ok_pg and ok_mg:
                    l1_pg, l2_pg = _eig(Sw, Sg + h)
                    l1_mg, l2_mg = _eig(Sw, Sg - h)
                    den_g = 2.0 * h
                elif ok_pg:
                    l1_pg, l2_pg = _eig(Sw, Sg + h)
                    l1_mg, l2_mg = l1, l2
                    ...
                    l1_mg, l2_mg = _eig(Sw, Sg - h)
                    den_g = h
                dl1_dSg = (l1_pg - l1_mg) / den_g
                dl2_dSg = (l2_pg - l2_mg) / den_g
